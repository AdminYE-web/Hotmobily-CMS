          ...
          for (var j = 0; j < input.files.length; j++) {
            ...
            reader.onload = (function (file) {
              return function (e) {
                ...
                newPreview.innerHTML = '<span>' + fileName + '</span>';
                message.appendChild(newPreview);
                if (fileData.startsWith('data:image')) {
                  var image = new Image();
                  image.src = fileData;
                  image.onload = function () {
                    var newIcon = document.createElement('div');
                    newIcon.className = 'file-drop-preview img-thumbnail rounded';
                    newIcon.innerHTML = '<img src="' + image.src + '" alt="' + fileName + '">';
                    newPreview.appendChild(newIcon);
                  };
                } else if (fileData.startsWith('data:video')) {
                  var newIcon = document.createElement('div');
                  newIcon.className = 'file-drop-icon ci-video';
                  newPreview.appendChild(newIcon);
                } else {
                  var newIcon = document.createElement('div');
                  newIcon.className = 'file-drop-icon ci-document';
                  newPreview.appendChild(newIcon);
                }
              };
            })(input.files[j]);
