	...
	function setToInput(){
		...
		if($('input[name=DeFormat]:checked').val()!=undefined){
			$('#prd_DeFormat').text('その他');
			$('#sample-prd-trace').text('その他');
			// $('input:hidden[name=DeFormat]').val('その他');
		}else{
			$('#prd_DeFormat').text('イラストレータファイル');
			$('#sample-prd-trace').text('イラストレータファイル');
			// $('input:hidden[name=DeFormat]').val('イラストレータファイル');
		}
		...
		tax = Math.floor(parseInt(TextField9Value) * 0.1);
...
function type_special_disabled(c){
	if(c == 't1'){
		...
		$.get("/count_order.php", function(data){
			...
			// $('input:hidden[name=DeFormat]').val('ご注文');
