...
// PLUSHIE_PDF_OPTIONS_START
function buildPlushiePdfOptions(partName, partUnitPrice, quantity) {
    var normalizedPartName = partName == null ? "" : String(partName);
    var hasPart = normalizedPartName !== "" && normalizedPartName !== "なし" && normalizedPartName !== "アタッチメントなし";
    var numericPartPrice = Number(partUnitPrice);
    var numericQuantity = Number(quantity);
    var attachmentRow = hasPart && Number.isFinite(numericPartPrice) && Number.isFinite(numericQuantity)
        ? "アタッチメント代金:" + numericQuantity + ":" + numericPartPrice
        : "";

    return {
        part: normalizedPartName,
        partPrice: 0,
        otherPrice: attachmentRow ? attachmentRow + ",版型代金:1:15500" : "版型代金:1:15500",
        delivery: "製作期間（試作含む）：36営業日",
        qoa: 0,
        deliveryPrice: 0,
    };
}
// PLUSHIE_PDF_OPTIONS_END
...
function validate(tmp) {
    ...
    //A
    var Item = $("#strap").val() || $("input[name='ItemType']").val() || "";
    ...
    var selectedPartName = selectedPart.val() || "";
    var selectedPartPrice = selectedPart.data("price");
    parts = selectedPartName === "なし" ? "" : selectedPartName;
    parts_price = Number(selectedPartPrice);
    if (!Number.isFinite(parts_price)) {
        parts_price = 0;
    }
    ...
    if (Item == "印刷ラバーストラップ・ラバーキーホルダー") {
        other_price =
            $("input[name=silk_print]:checked").val() +
            ":" +
            qty +
            ":" +
            print_charge / qty;
    ...
    if (Item == "ぬいぐるみポーチ") {
        ...
        var plushiePdfOptions = buildPlushiePdfOptions(parts, parts_price, qty);
        parts = plushiePdfOptions.part;
        parts_price = plushiePdfOptions.partPrice;
        other_price = plushiePdfOptions.otherPrice;
        delivery = plushiePdfOptions.delivery;
        ...
        success: function (data) {
            if (data == "success") {
                if (isSafari) {
                    ...
                    var pdfUrl = "/tcpdf/examples/pdf_export.php";
