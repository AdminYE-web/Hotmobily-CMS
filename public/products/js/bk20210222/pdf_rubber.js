...
function validate(tmp){
  ...
  var date = y+"年"+m+"月"+d+"日";
    ...
    if($('input[name="part"]:checked').val()!=undefined || $('input[name="part"]').val()!="なし"){
      ...
      parts_price = parts_obj["part_price"];
    }
    ...
    if($('#textfield_dis').length){
      var discount_quo = formatMoney($('#textfield_dis').val().replace(/,/g, ""));
    }
    var sum1_quo = formatMoney(parseInt($('#textfield9').val().replace(/,/g, ""))+parseInt(discount_quo.replace(/,/g, "")));
