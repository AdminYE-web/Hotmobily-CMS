$(function(){
    if(mobile_chk){
      ...
      $('#qty').replaceWith('<td class="TableRight" id="qty"><label>'+
        ...
        '<label class="control control--radio"><input type="radio" name="numberOf" id="no_of_order_0" class="right" onclick="next_area(\'numberOf\');show_input(\'show\');" value="0" '+select_0+'/>本数直接入力<span class="control__indicator"></span></label>'+
        '<div style="'+display_other+'" id="number_other"><input type="number" name="numberOfother" id="no_of_order_other" value="'+numberOfother+'" style="text-align:right;">本</div></label><div id="err_numberOf_mess">'+mrErrMsgList+'</div></td>');
      if(mode == "MODE_MOD"){
        $("#area2").show();
        $("#area3").show();
        $("#area4").show();
