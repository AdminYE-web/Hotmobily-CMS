...
function setToInput() {
  ...
  $.post('/products/ajax_calculate_price_pouch.php', buildPouchPayload(), function (response) {
...
function updateProductSummary() {
  ...
  $('#sample-prd-prdt').text(type + (option ? ' / ' + option : ''));
  $('#prd_production').text(type + (option ? ' / ' + option : ''));
  $('#sample-prd-size').text(size === '100' ? '100x180mm.' : '90x180mm.');
  $('#prd_size').text(size === '100' ? '100x180mm.' : '90x180mm.');
...
function calculateLocal() {
  ...
  var sample = $('input[name=cth_sample]:checked').val() === 'あり' ? 4950 : 0;
...
function fallbackProductCode() {
  var option = $('input[name=cth_option]:checked').val();
  var size = $('input[name=cth_size]:checked').val() === '100' ? '100' : '90';
  var grade = 'standard';
  if (option === 'プレミアム') {
    grade = 'premium';
  } else if (option === 'リサイクル材') {
    grade = 'recycle';
  }
  return 'pouch_' + grade + '_' + size;
}
...
function fallbackUnit(qty) {
  ...
  var table = {
    'スタンダード': {
      '90': [[100,385],[300,286],[500,253],[1000,231],[3000,220],[5000,209]],
      '100': [[100,396],[300,297],[500,264],[1000,242],[3000,231],[5000,220]]
    },
    'プレミアム': {
      '90': [[100,396],[300,297],[500,264],[1000,242],[3000,231],[5000,220]],
      '100': [[100,407],[300,308],[500,275],[1000,253],[3000,242],[5000,231]]
    },
    'リサイクル材': {
      '90': [[100,418],[300,319],[500,286],[1000,259],[3000,245],[5000,234]],
      '100': [[100,429],[300,330],[500,297],[1000,270],[3000,256],[5000,245]]
    }
  };
  var tiers = (((table[option] || {})[size]) || []);
  var unit = 0;
