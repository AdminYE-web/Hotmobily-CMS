...
function validate(tmp){
  ...
  var example_price = 0,ai_file_price = 0,back_print_price = 0;
    ...
    if($('input[name="part"]').length && $('input[name="part"]:checked').val()!=undefined || $('input[name="part"]').val()!="なし"){
    ...
    if($("input[name=coating]").length){
      ...
      item2 = "汚れ防止加工";
      delivery = "量産品製作期間：100～300個まで13営業日、配送：2日、300～1,000個まで17営業日、配送：2日、3,000個まで25営業日、";
    }
    ...
    if($('input[name=shape_type]').length && $('input[name=shape_type]:checked').val()!=undefined){
        shapeType = $('input[name=shape_type]:checked').val();
    }
