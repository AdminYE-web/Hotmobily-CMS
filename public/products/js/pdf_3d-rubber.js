...
function validate(tmp) {
    ...
    if (
        ($('input[name="acy_paper_select"]').length &&
            $('input[name="acy_paper_select"]:checked').val() == "あり") ||
        ($('input[name="paper_select"]').length &&
            $('input[name="paper_select"]:checked').val() == "あり")
    ) {
        paper = $('input[name="acy_paper"]:checked').val() || $('input[name="paper"]:checked').val();
    ...
    if (Item == "キャンディシール" || Item == "3D立体チャーム") {

        let candy_daishe_string = "なし";
        let trace_string = "なし";
        if ($("input[name=CandySticker_Daishe]:checked").val() !== undefined) {
            other_price = "台紙代金:" + qty + ":" + 33;
            candy_daishe_string = "あり";
        }   

        if ($("input[name=DeFormat]:checked").val() != undefined) {
            trace_string = "あり";
            // other_price += ",データ作成補助代金:" + 1 +":"+ 0;
        }
        
        if ($("#textfield2").length && $("#textfield2").val() != "") {
            let moldVal = $("#textfield2").val().replace(/,/g, "");
            if (parseInt(moldVal, 10) > 0) {
                if (other_price) {
                    other_price += ",版型代金:1:" + moldVal;
                } else {
                    other_price = "版型代金:1:" + moldVal;
                }
            }
        ...
        other += "台紙:" + candy_daishe_string;
        // other += ",データトレース:" + trace_string;
        // other += ",裏面印刷:" + $("input[name=silk_print]:checked").val();
        // other += ",本体色:" + $("input[name=ItemColor]:checked").val();
        ...
        delivery ="製作期間（試作含む）：28営業日、配送期間：1～2日";
    }
    ...
    $.ajax({
        ...
        data: {
            ...
            mold_price: $("#textfield2").val() || "195,000",
