...
function validate(tmp){
  ...
  var example_price = 0,ai_file_price = 0,back_print_price = 0;
    ...
    var part_prices_arr = new Array();

    if($('input[name="part"]').length && $('input[name="part"]:checked').val()!=undefined || $('input[name="part"]').val()!="カラビナなし"){
      part_arr.push($('input[name="part"]:checked').val());
      part_prices_arr.push(partPrice);

      if($('input[name=carabiner_print]:checked').val()=="刻印あり（表）" || $('input[name=carabiner_print]:checked').val()=="刻印あり（表+裏）"){
        part_arr.push($('input[name=carabiner_print]:checked').val());
        part_prices_arr.push(carabiner_print);
      }
    }

    if(part_arr.length > 0){
      parts = part_arr.toString();
      parts_price = part_prices_arr.toString();
    }

    if($('input[name="paper_select"]').length && $('input[name="paper_select"]:checked').val()!=undefined){
    ...
    if($("input[name=coating]").length){
      ...
      coating_price = formatMoney(Math.floor(coating_charge));
      item2 = "汚れ防止加工";
    ...
    if($('input[name="ItemPCS"]').length){
      if($('input[name="ItemPCS"]:checked').val() == "スタンダード"){
        ...
        delivery = "量産品製作期間：100～300個まで19営業日、配送：2日、300～1,000個まで21営業日、配送：2日、3,000個まで29営業日～、";
      }else if($('input[name="ItemPCS"]:checked').val() == "スタンダード（スピード7営業日発送）"){
