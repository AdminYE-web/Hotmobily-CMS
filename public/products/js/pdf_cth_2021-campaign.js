...
function validate(tmp){
  ...
  var date = y+"年"+m+"月"+d+"日";
    ...
    switch($('input[name=cth_type]:checked').val()){
      case "昇華転写片面印刷":Item = "オリジナルメガネクロス(昇華転写片面印刷)";break;
      case "昇華転写両面印刷":Item = "オリジナルメガネクロス(昇華転写両面印刷)";break;
      case "1色印刷（シルクスクリーン）":
      Item = "オリジナルメガネクロス(1色印刷（シルクスクリーン）)";
      itemtype_print = "版型代金";
      itemtype_print_price = "3,960";
      break;
      case "エンボス加工":
      Item = "オリジナルメガネクロス(エンボス加工)";
      itemtype_print = "版型代金";
      itemtype_print_price = "8,800";
      break;
    }
    ...
    $.ajax({
      ...
      data: {
        ...
        "filename": filename,
        "date": date,
        "remark": "生産国春節休暇に伴う納期遅延の為お値引設定中。期間外にご注文のお値引きは無効です。</span><br/><span>1月31日(月)～2月13日（日）のご注文分：5%",
        "save": "yes"
      },
      success: function(data){
        if(isSafari){
