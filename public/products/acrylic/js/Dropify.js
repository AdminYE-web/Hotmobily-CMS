...
(function (root, factory) {
  ...
  function Dropify(element, options) {
    ...
    var defaults = {
      ...
      messages: {
        default:
          "ç”»åƒãƒ•ã‚¡ã‚¤ãƒ«ã‚’æž å†…ã«ãƒ‰ãƒ­ãƒƒãƒ—ã™ã‚‹ã‹ã€ã“ã¡ã‚‰ã‚’ã‚¯ãƒªãƒƒã‚¯ã—ã¦ãã ã•ã„ã€‚",
        replace:
          "ç”»åƒã‚’å¤‰æ›´ã™ã‚‹å ´åˆã¯å†åº¦ã“ã¡ã‚‰ã«ãƒ•ã‚¡ã‚¤ãƒ«ã‚’ãƒ‰ãƒ©ãƒƒã‚°ã—ã¦ãã ã•ã„",
        remove: "å–ã‚Šæ¶ˆã—",
        error: "ã‚¨ãƒ©ãƒ¼ï¼å†åº¦ç”»åƒãƒ•ã‚¡ã‚¤ãƒ«ã‚’ã‚»ãƒƒãƒˆã—ã¦ãã ã•ã„ã€‚",
      },
      error: {
