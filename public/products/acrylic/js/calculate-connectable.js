...
function check_val(v) {
	if(v=='next'){
		...
		let ssh_display = false;

		let product_delivery = $('input[name="acy_delivery"]:checked').val();
		let product_size = $('input[name="acy_size"]:checked').val();
		let product_screen = $('input[name="acy_screen"]:checked').val();
		let product_unit = $('input[name="acy_cat"]:checked').val();

		switch (product_delivery) 
		{
			case '6営業日':

				if (product_unit == "アクリルパーツ2個") {
					max_qty = 150;
					error_qty = 'ご注文は1以上150以下でお願いします。';
				}

				if (product_unit == "アクリルパーツ3個") {
					max_qty = 100;
					error_qty = 'ご注文は1以上100以下でお願いします。';
				}

				if (product_unit == "アクリルパーツ4個") {
					max_qty = 75;
					error_qty = 'ご注文は1以上75以下でお願いします。';
				}

				if (product_unit == "アクリルパーツ5個") {
					max_qty = 60;
					error_qty = 'ご注文は1以上60以下でお願いします。';
				}
...
function chk_part(){
	...
	if($('input[name="acy_part_connectable"]:checked').val()==undefined){
		$('#part-connectable-error').text('接続パーツを選択してください');
		isValid = false;
	}else{
		$('#part-connectable-error').text('');
	}
...
function cal_c(v) {
	...
	var acrylic_number = $('input[name="acy_cat"]:checked').val();
	...
	if(part_connectable!=undefined){
		$('#sample-part2-pic').show();
		$('#sample-part2-pic').attr('src',parts_objc["part_pic"]);
		$('#sample-part2-name').text(parts_objc["part_name"]);
	}else{
		$('#sample-part2-pic').hide();
		$('#sample-part2-name').text('');
	}
	...
	let size_text = $('input[name="acy_size"]:checked').val()+'x'+$('input[name="acy_size"]:checked').val()+'mm';

	$('#sample-prd-delivery').text($('input[name="acy_delivery"]:checked').val());
	$('#sample-prd-size').text(size_text);
	$('#sample-prd-print').text($('input[name="acy_screen"]:checked').val());
	$('#sample-prd-part').text($('input[name="acy_cat"]:checked').val());
	$('#sample-prd-qty').text(qty);
	// $('#sample-prd-color').text(color);

	$('#prd_production').text($('input[name="acy_delivery"]:checked').val());
	$('#prd_size').text(size_text);
	$('#prd_print').text($('input[name="acy_screen"]:checked').val());
	$('#prd_part_con').text($('input[name="acy_cat"]:checked').val());
	$('#prd_qty').text(qty);
	$('#prd_amount').text(qty);
	$('#prd_part').text(part);
	...
	let bee = 0;
	if (acrylic_number != undefined)
	{
		if (acrylic_number == 'アクリルパーツ3個') {
			bee = Math.floor(198 * qty);
		}

		if (acrylic_number == 'アクリルパーツ4個') {
			bee = Math.floor(396 * qty);
		}

		if (acrylic_number == 'アクリルパーツ5個') {
			bee = Math.floor(594 * qty);
		}
	}

	acrylic_part_price = bee;

	// passed
	sub_total = Math.floor(((unit_price*qty)+trace_price+(partPrice*qty)+(paperPrice*qty)+(acrylic_part_price)));
	...
	$('#prd_price_acrylic_part').val(acrylic_part_price.toLocaleString("en"));
...
function validate(tmp){
  ...
  gtag('event', 'conversion', {
	...
	var acrylic_part = $('input[name="acy_cat"]:checked').val();
    ...
    $.ajax({
		...
		"acrylic_part":acrylic_part,
		"acrylic_part_price": acrylic_part_price,
        "paper": paper,
        "ai_file": ai_file,
        "qty": qty,
        //Price data
