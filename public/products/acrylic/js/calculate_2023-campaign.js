...
function cal_c(v) {
	...
	if(paper!=undefined){
		...
		if(paper_color!=undefined){
			...
			$('#sample-paper-pic').attr('src',"/products/acrylic/img/"+paper_color+".jpg?v=1.01");
	...
	vat_price = Math.floor((((unit_price*qty)*vat/(100+vat))+trace_price+((partPrice*qty)*vat/(100+vat))+((paperPrice*qty)*vat/(100+vat))-((disPrice*vat)/(100+vat))));
...
function writePriceTable(plus) {
	...
	function getPartData(v) {
		$.get("part_acrylic.php", { c: "passed" } , function(data){
