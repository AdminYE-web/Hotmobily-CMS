...
function cal_c(v) {
	...
	// passed
	var sub_total = Math.round(((unit_price*qty)+trace_price+(partPrice*qty)+(paperPrice*qty)));
	var vat = Math.round(sub_total*0.1);
	var total = Math.round(sub_total*1.1);
...
function validate(tmp){
    ...
    var sum1_quo = $('#prd_sub_total').val();
    var sum2_quo = $('#prd_sub_total').val();
    var sum3_quo = $('#vat').val();
    var sum4_quo = $('#prd_total').val();
    ...
    var parts_price = parts_obj["part_price"];
