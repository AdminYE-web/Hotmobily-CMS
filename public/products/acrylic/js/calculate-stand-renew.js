...
/**
 * Set initial default values for the summary table on page load,
 * matching the original stand.php behavior.
 */
$(function () {
    ...
    // Hide the coming-soon image when no paper is selected (default state)
...
function check_val(v) {
    if (v == 'next') {
        if ($('input[name=qty]').val() == "" || $('input[name=qty]').val() <= 0 || $('input[name=qty]').val() > 100) {
            ...
            if ($('input[name="acy_delivery"]:checked').val() == "6営業日" && $('input[name=qty]').val() > 100) {
                $('#qty-error').text('6営業日のご注文は100個以下となります');
                return false;
            } else {
                $('#qty-error').text('');
                cal_c();
 ...
 * Stand version: no part, no screen, has paper support.
 */
function cal_c(v) {
    ...
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_calculate_price_stand.php",
        dataType: "json",
        data: {
        ...
        success: function (response) {
            if (response.status == 200) {
                ...
                // Store paper unit price for PDF export
                paperPrice = response.paper_unit_price || 0;
 ...
 * Stand version: 商品代金 → 台紙 → 試作品 → データ作成補助 → 小計(税抜) → 消費税(10%) → 合計(税込)
 */
...
function calculate_price_manual() {
    ...
    return {
        ...
        part_price: 0,
        paper_price: Math.floor(paperPrice * parseInt(qty)),
        prototype_price: 0,
...
function writePriceTable(plus) {
    ...
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_price_table_stand.php",
        dataType: "json",
        data: {
            delivery: days
        },
