/**
 * calculate-hairbunch-renew.js
 * DB-based price calculator for アクリルヘアバンド (Acrylic Hairbunch).
 *
 * Key features:
 * - AJAX endpoints: ajax_calculate_price_hairbunch.php, ajax_price_table_hairbunch.php
 * - Part loading: part-bunch.php (via getPartData3)
 * - Product codes: acrylichairbunch_{delivery}_{side}
 * - Screen: always 片面印刷 (fixed, not selectable)
 * - Size: always 50 (50x50mm)
...
function cal_c(v) {
    ...
    // === AJAX call to get DB price (hairbunch endpoint) ===
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_calculate_price_hairbunch.php",
        dataType: "json",
        data: {
 ...
 * Uses part prices from parts_obj (loaded from part-bunch.php) to show
 * a meaningful price even when DB product codes aren't available yet.
 */
function cal_c_local() {
    console.warn('cal_c_local: Using local fallback (DB unavailable). Part prices from part-bunch.php.');
...
function writePriceTable(plus) {
    ...
    // AJAX call to get price table from DB (hairbunch endpoint — single size column)
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_price_table_hairbunch.php",
        dataType: "json",
        data: {
            delivery: days
        },
...
// === Part data loading (hairbunch uses part-bunch.php) ===
...
function getPartData3(v) {
    $.get("part-bunch.php", { c: "passed" }, function (data) {
...
function validate(tmp) {
    ...
    Item_print = $("input[name=acy_screen]").val() || '片面印刷';
