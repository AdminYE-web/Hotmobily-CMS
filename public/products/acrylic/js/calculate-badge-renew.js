/**
 * calculate-badge-renew.js
 * DB-based price calculator for アクリルクリップ (Badge/Clip).
 *
 * Based on calculate-umbrella-renew.js but with badge-specific differences:
 * - AJAX endpoints: ajax_calculate_price_badge.php, ajax_price_table_badge.php
 * - Part loading: part-acrylic-keychain.php (via getPartData3)
 * - Product codes: acrylicbadge_{delivery}_{side}_{size}
 * - Screen: dynamic from acy_screen radio (片面印刷 or 両面印刷)
 ...
 * Screen (印刷面) is read dynamically from the acy_screen radio input.
 */
function cal_c(v) {
    ...
    // === AJAX call to get DB price (badge endpoint) ===
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_calculate_price_badge.php",
        dataType: "json",
        data: {
 ...
 * Uses part prices from parts_obj (loaded from part-acrylic-keychain.php) to show
 * a meaningful price even when DB product codes aren't available yet.
 */
function cal_c_local() {
    console.warn('cal_c_local: Using local fallback (DB unavailable). Part prices from part-badge.php.');
    var qty = parseInt($('input[name="qty"]').val()) || 1;
    var part = $('input[name="acy_part"]:checked').val();

    // Part price from local parts_obj
    var localPartUnit = 0;
    var localPartTotal = 0;
    if (part != undefined && parts_obj && parseInt(parts_obj["part_price"]) > 0) {
        localPartUnit = parseInt(parts_obj["part_price"]);
        localPartTotal = localPartUnit * qty;
    }

    // Paper price from local paperPrice_obj (already computed in cal_c)
    var localPaperTotal = paperPrice * qty;

    sub_total = localPartTotal + localPaperTotal;
    disPrice = 0;
    vat_price = Math.floor(sub_total * 0.10);
    total = sub_total + vat_price;

    updatePriceDisplay(0, localPartTotal, localPaperTotal, 0, 0, sub_total, vat_price, disPrice, total);
}
...
function writePriceTable(plus) {
    ...
    // AJAX call to get price table from DB (badge endpoint)
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_price_table_badge.php",
        dataType: "json",
        data: {
            delivery: days,
            screen: $('input[name=prc_screen]:checked').val() || "1"
        },
        success: function (response) {
            if (response.status == 200) {
                var text = "";
...
// === Part data loading (badge uses part-acrylic-keychain.php) ===
...
function calculate_price_manual(type) {
    ...
    $.ajax({
        ...
        data: {
            ...
            prd_part_price: $('#prd_part_price').val(),
