...
// price for 10 days
var prd_10d_1side = { //Use price exclude vat
	"size50":[
	{"num":1, "price":978},
	{"num":2, "price":978},
	{"num":3, "price":978},
...
var prd_10d_2side = { //Use price include vat
	"size50":[
	{"num":1, "price":1329},
	{"num":2, "price":1329},
	{"num":3, "price":1329},
...
// price for 6 days
var prd_6d_1side = { //Use price exclude vat
	"size50":[
	{"num":1, "price":1077},
	{"num":2, "price":1077},
	{"num":3, "price":1077},
...
var prd_6d_2side = { //Use price include vat
	"size50":[
	{"num":1, "price":1438},
	{"num":2, "price":1438},
	{"num":3, "price":1438},
...
function check_val(v) {
	if(v=='next'){
		...
		//New option check processing
		$('#error_shape_processing').empty();
		if (!$('input[name="shape_processing"]').is(':checked')) {
...
function cal_c(v) {
	...
	$('#sample-prd-process').text($('input[name="shape_processing"]:checked').val());  //New option processing
	$('#sample-prd-screen').text(screen);
	$('#sample-prd-qty').text(qty);

	$('#prd_production').text(prodc_day);
	$('#prd_size').text(ItemSize+"mm"+"X"+ItemSize+"mm");
	$('#prd_processing').text($('input[name="shape_processing"]:checked').val()); //New option processing
	...
	var rule_type_b = false;
	if (($('input[name=acy_delivery]:checked').val() == "10営業日" && $('input[name=acy_screen]:checked').val() == "両面印刷") || ($('input[name=acy_delivery]:checked').val() == "6営業日" && $('input[name=acy_screen]:checked').val() == "両面印刷")) {
		rule_type_b = true;
	}
	...
	for (var i = 0, iMax = get_price().length; i < iMax; i++) {
		if (parseInt(qty) >= parseInt(get_price()[i].num)) {
			...
			if (rule_type_b) { //fix condition rule price
				// unit_price = Math.round(get_price()[i].price*(1+vat/100));
...
function calculate_price_manual(v) {
	...
	for (var i = 0, iMax = get_price().length; i < iMax; i++) {
		if (parseInt(qty) >= parseInt(get_price()[i].num)) {
			...
			unit_price = Math.floor(get_price()[i].price*(1+vat/100));
			if (rule_type_bs) { //fix condition rule price
				// unit_price = Math.round(get_price()[i].price*(1+vat/100));
				unit_price = Math.floor(get_price()[i].price);
			}
...
function writePriceTable(plus) {
			...
			for(var j=0;j<Object.size(price);j++){
				...
				for (var k = 0, iMax = price[tmp[j]].length; k < iMax; k++) {
					if (parseInt(amount[i]) >= parseInt(price[tmp[j]][k].num)) {
						...
						if (use_include) { //fix condition rule price
	...
	$(function() {			
		writePriceTable();
		if($("#strap").val()=="レインボーアクリルキーホルダー"){ //Load part for acrylic rainbow
	...
	function getPartData(v) {
		...
		$.get(part_url, { c: "passed" } , function(data){
			...
			let mmt = $('#ms_mode').val();
			if ((mmt != "" && mmt == "MODE_MOD") && (prd_strap == "レインボーアクリルキーホルダー")) //Only Acrylic Rainbow
			{
...
function setzero() {
	...
	$("#form")[0].reset();
	if($("#strap").val()=="アクリルキーホルダー" || $("#strap").val()=="レインボーアクリルキーホルダー"){ //Add Acrylic Rainbow
		getPartData($('input[name="acy_part"]:checked').val());
	}else if($("#strap").val()=="アクリルアンブレラマーカー"){
		getPartData2($('input[name="acy_part"]:checked').val());
	}else{
