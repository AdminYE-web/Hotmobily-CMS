...
function cal_c(v) {
    ...
    //check discount (effective 2026-08-11 to 2026-08-16)
    let now = new Date();
    let startDates2 = new Date("2026-08-11T00:00:00");
    let endDates2 = new Date("2026-08-16T23:59:59");
    if (now >= startDates2 && now <= endDates2 && $('input[name="acy_simulator"]:checked').val() == "はい") {
        disPrice = Math.floor(sub_total * 10/100);
    } else {
        disPrice = 0;
    }

    if ($('input[name="acy_simulator"]:checked').val() == "はい") {
        disPrice = Math.floor(sub_total * 10/100);
    } else {
        disPrice = 0;
    }
...
function calculate_price_manual(v) {
    ...
    //check discount (effective 2026-08-11 to 2026-08-16)
    let now = new Date();
    let startDates = new Date("2026-08-11T00:00:00");
    let endDates = new Date("2026-08-16T23:59:59");

    if (now >= startDates && now <= endDates && $('input[name="acy_simulator"]:checked').val() == "はい") {
        disPrice = Math.floor(sub_total * 10/100);
    } else {
        disPrice = 0;
    }
...
function writePriceTable(plus) {
	...
	function getPartData3(v) {
		$.get("part-badge.php", { c: "passed" } , function(data){
			...
			let prd_strap = $('#strap').val();
			let mmt = $('#ms_mode').val();
			if ((mmt != "" && mmt == "MODE_MOD") && (prd_strap == "アクリルバッジ" || prd_strap == "アクリルクリップ")) //Only Badge
			{
				let items = calculate_price_manual();
				if(Object.keys(items).length !== 0) 
				{
