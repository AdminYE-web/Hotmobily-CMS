...
function writePriceTable(plus) {
	...
	$(function() {			
		writePriceTable();
		if($("#strap").val()=="アクリルキーホルダー" || $("#strap").val()=="オーロラアクリルキーホルダー" || $('#strap').val() == "再生材料アクリルキーホルダー"){ //Add Acrylic Aurora, Recycle
	...
	function getPartData(v) {
		$.get("part.php", { c: "passed" } , function(data){
			...
			let mmt = $('#ms_mode').val();
			if ((mmt != "" && mmt == "MODE_MOD") && (prd_strap == "再生材料アクリルキーホルダー")) //Only Acrylic Recycle
			{
