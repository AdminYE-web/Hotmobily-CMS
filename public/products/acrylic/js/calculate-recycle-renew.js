/**
 * calculate-recycle-renew.js
 * Recycle Acrylic Keychain (再生材料アクリルキーホルダー) — DB-based price calculator.
 * Replaces hardcoded price arrays with AJAX calls to ajax_calculate_price_recycle.php.
 *
 * This file keeps the same UI logic (step1 → step2 → step3, validation, part selection)
 * as calculate_recycle.js, but fetches prices from the database instead of using JS price tables.
 */
...
function cal_c(v) {
    ...
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_calculate_price_recycle.php",
        dataType: "json",
        data: {
...
/**
 * Local fallback calculation (same as original calculate_recycle.js logic)
...
function writePriceTable(plus) {
    ...
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_price_table_recycle.php",
        dataType: "json",
        data: {
            delivery: days,
            screen: screen
...
$(function () {
    writePriceTable();
    // Recycle acrylic uses same parts as acrylic keychain
