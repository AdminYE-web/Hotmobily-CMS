/**
 * calculate-umbrella-renew.js
 * めじるしチャーム (Umbrella Marker) — DB-based price calculator.
 * Based on calculate-renew.js (keychain) but uses umbrella-specific AJAX endpoints.
 *
 * Differences from keychain:
 * - AJAX endpoints: ajax_calculate_price_umbrella.php, ajax_price_table_umbrella.php
 * - Part loading: part.php (shared umbrella parts)
 * - Product codes: acrylicumbrellamarker_{delivery}_{side}_{size}
...
// Paper prices (local fallback, same as keychain)
var paperPrice_obj = {
    "1side": [
...
function cal_c(v) {
    ...
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_calculate_price_umbrella.php",
        dataType: "json",
        data: {
 ...
 * Uses part prices from parts_obj (loaded from part.php) to show
 * a meaningful price even when DB product codes aren't available yet.
 */
function cal_c_local() {
    console.warn('cal_c_local: Using local fallback (DB unavailable). Part prices from part.php.');
...
function writePriceTable(plus) {
    ...
    // AJAX call to get price table from DB (umbrella endpoint)
    $.ajax({
        type: "POST",
        url: "/products/acrylic/ajax_price_table_umbrella.php",
        dataType: "json",
        data: {
            delivery: days,
            screen: screen
...
// === Part data loading (umbrella uses part.php) ===
...
function getPartData(v) {
    $.get("part-umbrella-renew.php", { c: "passed" }, function (data) {
...
// Alias for part.php loading (called from umbrella page onclick)
