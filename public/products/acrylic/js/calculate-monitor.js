...
function validation(c) {
  if (check_val(c)) {
    ...
    if (typeof event !== "undefined" && event && event.target) {
      if (
        document.referrer &&
        document.referrer == "https://hotmobily.jp/order/cart.php"
      ) {
    ...
    switch ($("#next").text()) {
      case "台座・オプション入力へ":
        if (c == "next" || c == "step2") {
          ...
          if (
            $('input[name="acy_size"]:checked').val() != "" &&
            $('input[name="acy_size"]:checked').val() !== undefined
          ) {
  ...
  if (v == "next") {
    if (
      ...
      if ($("input[name=qty]").val() < 20) {
        ...
        $("#sample-error").text(
          "※試作品をご希望される場合、ご注文納期とは別に試作品製作時間として6営業日+配送2日が必要になります。ご注文確定後に試作品の有無をご変更されるお客様が多くなっております。納期に余裕の無い場合、試作品のご依頼はご遠慮下さい。",
        );
        $("input[name=acy_sample]").attr("disabled", false);
      }
      if (
        $('input[name="acy_delivery"]:checked').val() == "6営業日" &&
        $("input[name=qty]").val() > 300
      ) {
...
function chk_part() {
  if ($('input[name="acy_part"]:checked').val() == undefined) {
    ...
    $("#part-error").text("背面パーツを選択してください。");
    validation("step2");
    return false;
  } else {
    $("#part-error").text("");
...
function cal_c(v) {
  ...
  if (part != undefined) {
    ...
    if (parts_obj["part_id"] == "base-cir") {
  partPrice = getBackPartUnitPrice(qty);
  $('#_base-cir').text(partPrice);
} else {
  ...
  if (paper != undefined) {
    ...
    if (paper_color != undefined) {
      ...
      $("#sample-paper-pic").attr(
        "src",
        "https://hotmobily.jp/products/acrylic/img/" +
          paper_color +
          ".jpg?v=1.01",
      );
  ...
  sub_total = Math.floor(
    unit_price * qty + trace_price + partPrice * qty + paperPrice * qty,
  );
  ...
  vat_price = Math.floor(
    unit_price * qty +
      trace_price +
      (partPrice * qty * vat) / (100 + vat) +
      (paperPrice * qty * vat) / (100 + vat),
  );
        ...
        case "片面印刷":
          switch ($('input[name="acy_size"]:checked').val()) {
            case "50":
              ...
              break;
            case "75":
              return prd_10d_1side["size75"];
              break;
            case "100":
              return prd_10d_1side["size100"];
              break;
          }
          break;
        case "両面印刷":
          switch ($('input[name="acy_size"]:checked').val()) {
            case "50":
              ...
              break;
            case "75":
              return prd_10d_2side["size75"];
              break;
            case "100":
              return prd_10d_2side["size100"];
              break;
          }
          break;
      }
        ...
        case "片面印刷":
          switch ($('input[name="acy_size"]:checked').val()) {
            case "50":
              ...
              break;
            case "75":
              return prd_6d_1side["size75"];
              break;
            case "100":
              return prd_6d_1side["size100"];
              break;
          }
          break;
        case "両面印刷":
          switch ($('input[name="acy_size"]:checked').val()) {
            case "50":
              ...
              break;
            case "75":
              return prd_6d_2side["size75"];
              break;
            case "100":
              return prd_6d_2side["size100"];
              break;
          }
          break;
      }
...
// Set amount
var amount_10d_base = ["1", "10", "50", "100", "200", "300", "500", "1000"];
var amount_6d_base = ["1", "10", "50", "100", "200", "300"];
...
function writePriceTable(plus) {
  ...
  // base amount ตาม納期
  var base_amount =
    days == "6" ? amount_6d_base.slice() : amount_10d_base.slice();

  // ถ้ากดลบ/reset ให้ล้างจำนวนที่ add เอง
  if (plus == "del") {
    amount_add = [];
  }
  ...
  if (plus == "add") {
    var newQty = $("input[name=new_qty]").val();

    if (
      $.inArray(newQty, base_amount) >= 0 ||
      $.inArray(newQty, amount_add) >= 0
    ) {
      ...
      if (newQty <= 0 || newQty > 1000) {
        ...
        if (days == "6" && (screen == "1" || screen == "2") && newQty > 300) {
          $(".add-error").text("6営業日のご注文は300個以下となります");
        } else {
          $(".add-error").text("");
          amount_add.push(newQty);
        }
      }
    }
  }

  // รวม base + added แล้วเรียง
  amount = base_amount.concat(amount_add);
    ...
    for (var j = 0; j < Object.size(price); j++) {
      ...
      for (var k = 0, iMax = price[tmp[j]].length; k < iMax; k++) {
        if (parseInt(amount[i]) >= parseInt(price[tmp[j]][k].num)) {
          price1 = Math.floor(price[tmp[j]][k].price);
          continue;
        }
        break;
      }
...
function getPartData() {
  $.get(
    "https://hotmobily.jp/products/acrylic/part-base-monitor.php",
    { c: "passed", s: $('input[name="acy_size"]:checked').val() },
    function (data) {
      var duce = $.parseJSON(data);
      for (var i = 0; i < duce.length; i++) {
        if (
          duce[i]["part_id"] == $('input[name="acy_part"]:checked').attr("id")
        ) {
          parts_obj = duce[i];
          break;
        } else {
    ...
    if (
      $("#ms_mode").val() != "" &&
      $("#ms_mode").val() == "MODE_MOD" &&
      prd_strap == "アクリルフィギュアスタンド"
    ) //Only Figure
    {
...
function chk_base() {
  ...
  $(".base_group.picpro").each(function () {
    ...
    $(this)
      .parent()
      .find(".part_price")
      .text(part_name_global[i]["part_price"] * 1.1);
    $(this).parent().find("input").val(part_name_global[i]["part_name"]);
    i++;
  });
}

function getBackPartUnitPrice(qty) {
...
function updateBackPartDisplayPrice() {
  var qty = parseInt($('input[name="qty"]').val() || 0, 10);
  var price = getBackPartUnitPrice(qty);

  $("#_base-cir").text(price);
}
$(function () {
  updateBackPartDisplayPrice();

  $('input[name="qty"]').on("input change keyup", function () {
    updateBackPartDisplayPrice();
    cal_c();
  });
});
