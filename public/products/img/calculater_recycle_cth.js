		...
		switch(type){
			...
			case 'i2':
				...
				for(rows=0;rows<6;rows++){
					if(!document.getElementById("pack_have").checked){
						document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.ceil(((numUnitPrice[type+"_"+size][rows]["price"] * qty[rows])+sum)/qty[rows]))+"円";
						document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(Number((numUnitPrice[type+"_"+size][rows]["price"] * qty[rows])+sum))+"円";
						tax = Math.floor((numUnitPrice[type+"_"+size][rows]["price"] * qty[rows] + sum) * 1.1);
						document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(tax)+"円";
					}else{
						document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.ceil(((numUnitPrice[type+"_"+size][rows]["price"] * qty[rows])+sum+(qty[rows]*10))/qty[rows]))+"円";
						document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(Number((numUnitPrice[type+"_"+size][rows]["price"] * qty[rows])+sum+(qty[rows]*10)))+"円";
						tax = Math.floor((numUnitPrice[type+"_"+size][rows]["price"] * qty[rows] + sum +(qty[rows]*10)) * 1.1);
						document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(tax)+"円";
					}
				}
			break;			
