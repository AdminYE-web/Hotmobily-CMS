	...
	var price_rubber_frame = [1300,1060,770,650,468,354,298];
		...
		if(v=='next'){
			if(!validation_numberOf() && $('input[name=ItemPCS]:checked').val()!="ホットモバイリーファン"){
				...
				if($('input[name=ItemPCS]').length && $('input[name=ItemPCS]:checked').val()==undefined){
					...
					return false;
				}else if($('input[name=silk_print]').length && ItemType != "ラバータグ" && $('input[name=silk_print]:checked').val()==undefined){
	...
	function setToInput(){
		...
		//phone stand
			...
			if ($('#pcs1').is(':checked') && $('input[name=ItemSize]:checked').val()=='小サイズ'){
			...
			if ($('#pcs1').is(':checked') && $('input[name=ItemSize]:checked').val()=='大サイズ'){
		...
		if($('input[name=silk_print]').length && ItemType != "ラバータグ" && $('input[name=silk_print]:checked').val()!=undefined){
		...
		if($('#pcs1').length){
			if($('#pcs1').is(':checked') && ItemType != "スマホラバークリーナー" && ItemType != "ラバータグ"){
				if($('input[name=SendPrototype]:checked').val()!=undefined){ proto_charge = Math.floor(0*(1+vat/100)); }
				if($('input[name=DeFormat]:checked').val()!=undefined){ trace_charge = Math.floor(0*(1+vat/100)); }
				if(print_umu.checked){ print_charge = (vno_of_order.value)*Math.floor(33*(1+vat/100));}
				click_typeorder_enabled();
			}else if($('#pcs1').is(':checked') && ItemType != "スマホラバークリーナー" && ItemType == "ラバータグ"){
				if($('input[name=SendPrototype]:checked').val()!=undefined){ proto_charge = Math.floor(0*(1+vat/100)); }
				if($('input[name=DeFormat]:checked').val()!=undefined){ trace_charge = Math.floor(0*(1+vat/100)); }
				click_typeorder_enabled();
			}else if($('#pcs1').is(':checked') && ItemType == "スマホラバークリーナー" && ItemType != "ラバータグ"){
		...
		(paperPrice*vno_of_order.value) + + packing_price*vno_of_order.value;

		DisPrice = 0;

		// DisPrice += Math.floor(TextField9Value*(5/100));
...
function getPartData(v) {
	if($('input[name=campaign]').val()=="rubber_strap"){
	...
	}else if($('input[name=campaign]').val()=="rubber_keyholder"){
		$.get("part_keyholder.php", { c: "passed" } , function(data){
			var duce = $.parseJSON(data);
			for(var i=0;i<duce.length;i++){
				if(duce[i]['part_name']==v){
					parts_obj = duce[i];
