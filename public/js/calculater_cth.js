...
var numUnitPrice;
 numUnitPrice = {
	...
	function calc(val) {
		...
		if(itemType==="i3"){
	      basic_charge = Math.floor(3600*(1+vat/100));
	    }else if(itemType==="i4"){
	      basic_charge = Math.floor(8000*(1+vat/100));
	    }else{
		  basic_charge = 0;
	    }
		...
		switch(type){
			...
			case 'i4':
				...
				for(rows=0;rows<6;rows++){
					if(!document.getElementById("pack_have").checked){
						total = Math.floor((Math.floor(numUnitPrice[type+"_"+type2+"_"+size][rows]["price"]*(1+vat/100)) * qty[rows] + sum));
					}else{
						total = Math.floor((Math.floor(numUnitPrice[type+"_"+type2+"_"+size][rows]["price"]*(1+vat/100)) * qty[rows] + sum +(qty[rows]*Math.floor(10*(1+vat/100)))));
					}
			...
			case 'i3':
				...
				for(rows=1;rows<6;rows++){
					if(!document.getElementById("pack_have").checked){
						total = Math.floor(((Math.floor(numUnitPrice[type+"_"+type2+"_"+size][rows]["price"]*(1+vat/100))+(colors*Math.floor(10*(1+vat/100)))) * qty[rows] + sum));
					}else{
						total = Math.floor(((Math.floor(numUnitPrice[type+"_"+type2+"_"+size][rows]["price"]*(1+vat/100))+(colors*Math.floor(10*(1+vat/100)))) * qty[rows] + sum+(qty[rows]*Math.floor(10*(1+vat/100)))));
					}
					document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.floor(total/qty[rows]))+"円";
					document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(total)+"円";
					row1++;
  ...
  function WritePriceTable() {
	...
	if(type==="i3"){
		for(var o=2,i=1;o<=6;o++,i++){
			document.getElementById('CLO_'+o).innerHTML = formatMoney(Math.floor(this.loop1[i].price*(1+vat/100))+(colors*Math.floor(10*(1+vat/100))));
		}
