/**
 * calculate_acrylic_badge_admin.js
 * Admin-side pricing calculator for アクリルバッジ.
 *
 * Sourced dynamically from database pricing:
 *   - Fetches rates dynamically from window.numUnitPrice['acrylicbadge_...'"]
 *   - Same paperPrice_obj and calculations as calculate_acrylic_umbrella_admin.js
...
function get_price_admin() {
    ...
    // アクリルバッジ is single-sided only
    var s_key = '1side';

    var size = $('input[name="acy_size"]:checked').val() || '50';
    var dbKey = 'acrylicbadge_' + d_key + '_' + s_key + '_' + size;
...
function cal_c() {
    ...
    var sizeVal = $('input[name="acy_size"]:checked').val() || '50';
    var ItemSize = sizeVal + 'mm × ' + sizeVal + 'mm';

    var ItemScreen = '片面印刷'; // fixed
    ...
    if (qty <= 0 || qty > 1000) {
        $('#error_message').text('ご注文は1以上1,000以下でお願いします。');
    ...
    $('#prd_screen').text(ItemScreen);
    $('#prd_qty').text(qty);

    if (qty <= 0 || qty > 1000) {
        $('#error_message').html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> ご注文は1以上1,000以下でお願いします。');
