...
function get_price_admin() {
    ...
    var dbKey = 'acrylicfigstand_' + d_key + '_' + p_key + '_' + size;
...
function calcAcrylic() {
    ...
    if (part !== undefined && part !== '') {
        if (part === '【オリジナル】印刷あり') {
            var dbKey = '【オリジナル】印刷あり_' + size_val;
            var tmp_part = window.numUnitOptionPrice ? window.numUnitOptionPrice[dbKey] : [];
            if (!tmp_part || tmp_part.length === 0) {
                tmp_part = window.numUnitOptionPrice ? window.numUnitOptionPrice[dbKey + '_0'] : [];
            }
            if (tmp_part && tmp_part.length > 0) {
                partPrice = Math.floor(tmp_part[0].price);
            } else {
                if (size_val === '50') partPrice = 20;
                else if (size_val === '75') partPrice = 40;
                else if (size_val === '100') partPrice = 80;
                else partPrice = 20;
            }
        } else if (part.indexOf('【定型】') !== -1 || part === '【オリジナル】印刷なし') {
            partPrice = 0;
        } else {
            var partKey = part;
    ...
    $('#textfield7').val(Math.floor(unit_price * qty));
    $('#unit_price').val(unit_price);

    $('#partPrice_disp').val((partPrice * qty).toLocaleString("en"));
    $('#acy_part_total').val(partPrice * qty);

    $('#acy_prototype_price').val(sample_price);
    $('#textfield3').val(sample_price);
    $('#acy_trace_price').val(trace_price);
    ...
    $('#textfield7_2').val(paperPrice * qty);
}
