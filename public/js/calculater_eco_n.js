...
function getObjects() {
	...
	function getPrice(){
		...
		if(validateCheck()) {
			...
			$('#button_pdf').replaceWith('<input id="button_pdf" type="button" style="cursor:pointer;height: 30px;width: 250px;" onclick="javascript:validate2();$(\'.loading\').show();" value="御見積書PDF出力（社名記載なし）"/>');
  ...
  function validateCheck() {
		...
		if(!getItemPrint()){
			document.getElementById('err_no_of_ItemSpeed').innerHTML = "<span style=color:red;>※印刷方法をご指定下さい。</span>";
			error = false;
		}
		return error;
	}
	...
	function clearErrFields() {
		...
		document.getElementById('err_no_of_ItemSpeed').innerHTML = "";
	}
	...
	function calc() {
		...
		if(itemPrint=="0") {
			for(var i = 0;i<numUnitPrice[itemType+"_"+itemPrint].length;i++){
				if(qty>=numUnitPrice[itemType+"_"+itemPrint][i]["num"]){
					type_price = (Math.floor(numUnitPrice[itemType+"_"+itemPrint][i]["price"]*(1+vat/100))+($("#ItemColor").val()*Math.floor(10*(1+vat/100))))*qty;
					continue;
				}
				break;
			}
			($("#ItemColor").val()==0)?silk_print = Math.floor(6000*(1+vat/100)):silk_print = Math.floor(6000*(1+vat/100))*(itemColor+1);
		}
		else{
			for(var i = 0;i<numUnitPrice[itemType+"_"+itemPrint].length;i++){
				if(qty>=numUnitPrice[itemType+"_"+itemPrint][i]["num"]){
					type_price = Math.floor(numUnitPrice[itemType+"_"+itemPrint][i]["price"]*(1+vat/100)) * qty;
					continue;
				}
				break;
			}
			silk_print = Math.floor(65000*(1+vat/100));
		}		
		if($("input[name=ItemSpeed]:checked").val()==0){
			proto_charge = Math.floor((type_price+silk_print)*0.1*(1+vat/100));
		}
		...
		tax = Math.floor(type_price*vat/(100+vat))+Math.floor(silk_print*vat/(100+vat))+Math.floor(proto_charge*vat/(100+vat));
...
function click_itemtype() {
	...
	if($("input[name='ItemPrint']:checked").val() == 0){
		for (var j = 0, k = numUnitPrice[itemType+"_"+print].length; j < k ; j++) {
			document.getElementById( 'PD_' + parseInt(j+1) ).innerHTML = 
			formatMoney(parseInt(Math.floor(numUnitPrice[itemType+"_"+print][j]["price"]*(1+vat/100))+($("#ItemColor").val()*Math.floor(10*(1+vat/100)))));
		}
	}else if($("input[name='ItemPrint']:checked").val() == 1){
		for (var j = 0, k = numUnitPrice[itemType+"_"+print].length; j < k ; j++) {
			document.getElementById( 'PD_' + parseInt(j+1) ).innerHTML = 
			formatMoney(Math.floor(numUnitPrice[itemType+"_"+print][j]["price"]*(1+vat/100)));
		}
	}else{
		for (var j = 0, k = numUnitPrice[itemType+"_0"].length; j < k ; j++) {
			document.getElementById( 'PD_' + parseInt(j+1) ).innerHTML = 
			formatMoney(Math.floor(numUnitPrice[itemType+"_0"][j]["price"]*(1+vat/100)));
		}
	}
	switch(itemType){
...
function chg_itemColor() {
	...
	function table_priceW(){
		...
		var sum = 0;
		var proto_charge = 0;
		if($('input[name=ItemPrint]:checked').val() == "1"){
			silk_print = Math.floor(65000*(1+vat/100));
		}else{
			silk_print = Math.floor(6000*(1+vat/100))*(itemColor+1);
		}
		// if 0 customer want speed production
		if($('input[name=ItemSpeed]:checked').val() == "0"){
			for(rows=0;rows<6;rows++){
				sum = Math.floor(((parseInt(Math.floor(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]*(1+vat/100))+($("#ItemColor").val()*Math.floor(10*(1+vat/100))))*qty[rows])+silk_print)*0.1);
				document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.ceil(Number((parseInt(Math.floor(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]*(1+vat/100))+($("#ItemColor").val()*Math.floor(10*(1+vat/100))))*qty[rows])+silk_print+sum)/qty[rows]))+"円";
				total = Math.floor(Number((parseInt(Math.floor(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]*(1+vat/100))+($("#ItemColor").val()*Math.floor(10*(1+vat/100))))*qty[rows])+silk_print+sum));
				document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(total)+"円";
			}
		}else{
			for(rows=0;rows<6;rows++){
				document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.floor(Number((parseInt(Math.floor(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]*(1+vat/100))+($("#ItemColor").val()*Math.floor(10*(1+vat/100))))*qty[rows])+silk_print)/qty[rows]))+"円";
				total = Math.floor(Number((parseInt(Math.floor(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]*(1+vat/100))+($("#ItemColor").val()*Math.floor(10*(1+vat/100))))*qty[rows])+silk_print));
				document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(total)+"円";
			}
		}	
	}
...
function clear_field(){
  ...
  function write_price(){
  	for (var j = 0, k = numUnitPrice["ec1_0"].length; j < k ; j++) {
		document.getElementById( 'PD_' + parseInt(j+1) ).innerHTML = formatMoney(Math.floor(numUnitPrice["ec1_0"][j]["price"]*(1+vat/100)));
	}
	$('#type_it').text('円(EC-1)');
  }
  ...
  function clear_v(){
  	$('#no_of_order').val('')
  	$('input[name=ItemPrint]').attr('checked',false);
  	$('#itemColors').hide();
  	$('#ec1').show();
  	for(var i=2;i<=8;i++)$('#ec'+i).hide();
  	$('#ItemType option:first-child').attr("selected", "selected");
  }