/**
 * calculate_acrylic_umbrella_admin.js
 * Admin-side pricing calculator for アクリルアンブレラマーカー.
 *
 * Sourced dynamically from database pricing:
 *   - Fetches rates dynamically from window.numUnitPrice['acrylicumbrellamarker_...']
...
function get_price_admin() {
    ...
    var s_key = (screen === '両面印刷') ? '2side' : '1side';

    var size = $('input[name="acy_size"]:checked').val() || '50';
    var dbKey = 'acrylicumbrellamarker_' + d_key + '_' + s_key + '_' + size;
...
function cal_c() {
    ...
    var ItemScreen = $('input[name="acy_screen"]:checked').val() || '片面印刷';
    ...
    if (qty <= 0 || qty > 1000) {
        $('#error_message').text('ご注文は1以上1000以下でお願いします。');
    ...
    if (qty <= 0 || qty > 1000) {
        $('#error_message').html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> ご注文は1以上1000以下でお願いします。');
