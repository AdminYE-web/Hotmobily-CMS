/**
 * calculate_acrylic_omamori_admin.js
 * Admin-side pricing calculator for お守りアクリルキーホルダー (Omamori Acrylic Keyholder).
 *
 * Sourced dynamically from database pricing:
 *   - Fetches rates dynamically from window.numUnitPrice['recycle_acrylickeyholder_...']
 *   - Same paperPrice_obj and calculations as customer-side calculate_omamori_keyholder.js
...
function get_price_admin() {
    ...
    var p_key = '1side';
    if (print === '両面印刷：オーロラ加工なし') p_key = '2side';
    if (print === '片面印刷・オーロラ加工あり') p_key = '3side';
    var dbKey = 'acrylickeyholder_omamori_' + d_key + '_' + p_key;
...
function cal_c() {
    ...
    if (size_val.indexOf('75x50mm') !== -1) {
        $('.type_b').css('display', 'none');
        $('.type_a').css('display', 'flex');
        if ($('input[name="acy_part"]:checked').closest('.part-card-label').hasClass('type_b')) {
            $('input[name="acy_part"][value="なし"]').prop('checked', true);
        }
    } else if (size_val.indexOf('70x42mm') !== -1) {
        $('.type_a').css('display', 'none');
        $('.type_b').css('display', 'flex');
        if ($('input[name="acy_part"]:checked').closest('.part-card-label').hasClass('type_a')) {
            $('input[name="acy_part"][value="なし"]').prop('checked', true);
        }
    }
