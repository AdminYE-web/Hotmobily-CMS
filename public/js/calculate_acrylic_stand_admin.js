/**
 * calculate_acrylic_stand_admin.js
 * Admin-side pricing calculator for アクリルスマホスタンド.
 *
...
function get_price_admin() {
    ...
    var dbKey = 'acrylicstand_' + d_key + '_1side';
...
function cal_c() {
    ...
    if (qty <= 0 || qty > 100) {
        $('#error_message').text('ご注文は1以上100以下でお願いします。');
    ...
    if (qty <= 0 || qty > 100) {
        $('#error_message').html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> ご注文は1以上100以下でお願いします。');
