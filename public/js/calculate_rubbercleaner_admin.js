...
/* Product-local calculator helpers for create-order-rubbercleaner.php. */
function rbCleanerReadPrice(pool, key, qty) {
...
function rbCleanerAddPrice(code, qty) {
    ...
    var service = normalized.replace(/^smartphonecleaner_/, '');
    var candidates = [];
    function add(key) { if (key && candidates.indexOf(key) === -1) candidates.push(key); }
    add(requested); add(normalized); add('smartphonecleaner_' + service); add(service);
    ...
    for (var i = 0; i < candidates.length; i++) {
        var price = rbCleanerReadPrice(pool, candidates[i], qty || 1);
        if (price !== null) return price;
    }
    return 0;
}

function rbCleanerManualTotal() {
 ...
 */
function get_cleaner_price() {
    var itemPCS = $('input[name="ItemPCS"]:checked').val() || 'スタンダード';
    var dbKey = (itemPCS === 'プレミアム')
        ? 'smartphonecleaner_rubber_premium'
        : 'smartphonecleaner_rubber_standard';
...
function get_add_price(code) {
    return rbCleanerAddPrice(code, 1);
}
...
function setToInput() {
    ...
    var backColor = $('input[name="back_side_color"]:checked').val() || '';
    ...
    var price_tiers = get_cleaner_price();
    ...
    // ---- 3. Coating price ----
    coatingPrice = 0;
    if (coating === '汚れ防止加工' || coating === '汚れ防止加工あり') {
        coatingPrice = get_add_price('smartphonecleaner_antistain') || get_add_price('anti_stain') || get_add_price('coating') || get_add_price('rubber_coating');
    }
    ...
    if (isSendPrototype) {
        proShipping = get_add_price('smartphonecleaner_prototype') || get_add_price('rubber_prototype') || get_add_price('prototype');
    ...
    if (isDeFormat) {
        traceCharge = get_add_price('smartphonecleaner_datatrace') || get_add_price('rubber_data_trace') || get_add_price('data_trace');
    ...
    if (designNum > 1) {
        var variationUnitPrice = get_add_price('smartphonecleaner_colorvariation') || get_add_price('rubber_color_variation') || get_add_price('color_variation');
    ...
    var materialKey = '';
    switch (itemMaterial) {
        case '特殊素材：蓄光（オレンジ色）': materialKey = 'smartphonecleaner_phosphorescent_orange'; break;
        case '特殊素材：蓄光（緑色）': materialKey = 'smartphonecleaner_phosphorescent_green'; break;
        case '特殊素材：蛍光': materialKey = 'smartphonecleaner_fluorescent'; break;
        case '特殊素材：ラメ': materialKey = 'smartphonecleaner_glitter'; break;
        case '特殊素材：金色': materialKey = 'smartphonecleaner_gold'; break;
        case '特殊素材：銀色': materialKey = 'smartphonecleaner_silver'; break;
    }
    if (materialKey !== '') {
    ...
    // ---- Subtotal ----
    var manualServiceTotal = rbCleanerManualTotal();
    sub_total = strapPrice + partTotal + coatingTotal + paperTotal + proShipping + traceCharge + designsCharge + materialCharge + manualServiceTotal;
    disPrice = 0;
    total = sub_total - disPrice;

    // ---- Summary display (spec table) ----
    $('#prd_ItemPCS').text(itemPCS);
    $('#prd_cloth').text(backColor);
