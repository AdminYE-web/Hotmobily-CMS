...
function chg_itemColor() {
	...
	function table_priceW(){
		...
		if($('input[name=ItemPrint]:checked').val() == "1"){
			...
			silk_print = 6000*(itemColor+1);
		}
		// if 0 customer want speed production
		if($('input[name=ItemSpeed]:checked').val() == "0"){
			for(rows=0;rows<6;rows++){
				sum = Math.floor(((parseInt(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]+($("#ItemColor").val()*10))*qty[rows])+silk_print)*0.1);
				document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.ceil(Number((parseInt(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]+($("#ItemColor").val()*10))*qty[rows])+silk_print+sum)/qty[rows]))+"円";
				document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(Number((parseInt(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]+($("#ItemColor").val()*10))*qty[rows])+silk_print+sum))+"円";
				tax = Math.floor(Number((parseInt(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]+($("#ItemColor").val()*10))*qty[rows])+silk_print+sum)*1.1);
				document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(tax)+"円";
			}
		}else{
			for(rows=0;rows<6;rows++){
				document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.ceil(Number((parseInt(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]+($("#ItemColor").val()*10))*qty[rows])+silk_print)/qty[rows]))+"円";
				document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(Number((parseInt(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]+($("#ItemColor").val()*10))*qty[rows])+silk_print))+"円";
				tax = Math.floor(Number((parseInt(numUnitPrice[itemType+"_"+itemPrint][rows]["price"]+($("#ItemColor").val()*10))*qty[rows])+silk_print)*1.1);
				document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(tax)+"円";
			}
		}	
	}
