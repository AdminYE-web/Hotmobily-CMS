function get_price_admin() {
    ...
    if (window.numUnitPrice) {
        ...
        for (var i = 0; i < keys.length; i++) {
            var k = keys[i];
            if (k.indexOf('flight_tag_badge') !== -1) return window.numUnitPrice[k];
        }
    }
...
function calcFlightTagBadge() {
    ...
    if ($("select[name=ft_size] option:selected").val() != undefined) {
        order_pcs = order_pcs + 15 * ($("select[name=ft_size] option:selected").val() - 5);
    }
    ...
    if ($("input[name=ft_process]:checked").val() == "オーバーロック仕上げ") {
        process_price = 50 * vno_of_order;
    }

    var backside_price = 0;
    var backsideType = $("input[name=ft_backside]:checked").val();
    if (backsideType == "ロック式の安全ピン" || backsideType == "バタフライクラッチ") {
        backside_price = 10 * vno_of_order;
    }

    var color_variation_price = 0;
    var cv = parseInt($("input[name=color_variation]").val(), 10) || 1;
    if (cv > 3) {
        color_variation_price = (cv - 3) * 20 * vno_of_order;
    }
...
window.calcFlightTagBadge = calcFlightTagBadge;
if (window.__calcFlightTagBadgeQueue && window.__calcFlightTagBadgeQueue.length) {
    window.__calcFlightTagBadgeQueue.forEach(function (args) { calcFlightTagBadge.apply(null, args); });
    window.__calcFlightTagBadgeQueue = [];
