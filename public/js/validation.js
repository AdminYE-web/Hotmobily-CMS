...
function validation_numberOf() {
	...
	if(screen.width<=768){
		if(document.querySelector("input[name=numberOf]:checked").value=='0'){
			var vNumberOfOder = document.getElementById('no_of_order_other');
			if(vNumberOfOder.value<=500){
				mess += "<font color='red'>本数は500本以上で入力して下さい</font>";
				err_number.innerHTML = mess;
				return false;
			}
		}else{
			var vNumberOfOder = document.querySelector("input[name=numberOf]:checked");
		}
	}else{
