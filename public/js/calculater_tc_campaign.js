...
function getObjects() {
	...
	function calc(val) {
		...
		if(itemType == "target" && itemQA =="standard") {
			...
			if(qty>=1 && qty<=199 ) {
				strap_price = numUnitPrice["target"][0]["price"] * qty;
			}
			else if(qty>=200 && qty<=299 ) {
				strap_price = numUnitPrice["target"][1]["price"] * qty;
			}
			else if(qty>=300 && qty<=499 ) {
				strap_price = numUnitPrice["target"][2]["price"] * qty;
			}
			else if(qty>=500 && qty<=999 ) {
				strap_price = numUnitPrice["target"][3]["price"] * qty;
			}
			else if(qty>=1000 && qty<=2999 ) {
				strap_price = numUnitPrice["target"][4]["price"] * qty;
			}
			else if(qty>=3000 && qty<=4999 ) {
				strap_price = numUnitPrice["target"][5]["price"] * qty;
			}
			else if(qty>=5000 ) {
				strap_price = numUnitPrice["target"][6]["price"] * qty;
			}
		}
		else if(itemType == "target" && itemQA =="premium") {

			if(qty>=1 && qty<=199 ) {
				strap_price = numUnitPrice["target_premium"][0]["price"] * qty;
			}
			else if(qty>=200 && qty<=299 ) {
				strap_price = numUnitPrice["target_premium"][1]["price"] * qty;
			}
			else if(qty>=300 && qty<=499 ) {
				strap_price = numUnitPrice["target_premium"][2]["price"] * qty;
			}
			else if(qty>=500 && qty<=999 ) {
				strap_price = numUnitPrice["target_premium"][3]["price"] * qty;
			}
			else if(qty>=1000 && qty<=2999 ) {
				strap_price = numUnitPrice["target_premium"][4]["price"] * qty;
			}
			else if(qty>=3000 && qty<=4999 ) {
				strap_price = numUnitPrice["target_premium"][5]["price"] * qty;
			}
			else if(qty>=5000 ) {
				strap_price = numUnitPrice["target_premium"][6]["price"] * qty;
			}
		}
		...
		if($('input[name=carabiner]:checked').val()=="1")
		{
			//clear color 
			for( var i=1; i<=7; i++ ){
				document.getElementById( 'CBO_' + i ).style.background = "";
				document.getElementById( 'CBM_' + i ).style.background = "";
				document.getElementById( 'CBL_' + i ).style.background = "";
			}
			var carabiner_txt = "";
			carabiner_print = 0;
			carabiner_price = 0;
			...
			switch($('input[name=carabiner_type]:checked').val()){
				case '廉価版カラビナ':
					...
					carabiner_fld = "CBO_";
					$('#pricetable_carabiner_o').show();
				break;

				case '高級版Mサイズ':
					$('#pricetable_carabiner_m').show();
					carabiner_txt = "carabiner_M";
					carabiner_fld = "CBM_";
					if($('input[name=carabiner_print]:checked').val()=="1") {carabiner_print = 20 * qty;}
					else if($('input[name=carabiner_print]:checked').val()=="2") {carabiner_print = 30 * qty;}
				break;

				case '高級版Lサイズ':
					$('#pricetable_carabiner_l').show();
			...
			//Get carabiner price
			if(qty>=1 && qty<=199 ) {
				carabiner_price = numCarabinerPrice[carabiner_txt][0]["price"] * qty;
				document.getElementById( carabiner_fld + "1" ).style.background = "pink";
			}
			else if(qty>=200 && qty<=299 ) {
				carabiner_price = numCarabinerPrice[carabiner_txt][1]["price"] * qty;
				document.getElementById( carabiner_fld + "2" ).style.background = "pink";
			}
			else if(qty>=300 && qty<=499 ) {
				carabiner_price = numCarabinerPrice[carabiner_txt][2]["price"] * qty;
				document.getElementById( carabiner_fld + "3" ).style.background = "pink";
			}
			else if(qty>=500 && qty<=999 ) {
				carabiner_price = numCarabinerPrice[carabiner_txt][3]["price"] * qty;
				document.getElementById( carabiner_fld + "4" ).style.background = "pink";
			}
			else if(qty>=1000 && qty<=2999 ) {
				carabiner_price = numCarabinerPrice[carabiner_txt][4]["price"] * qty;
				document.getElementById( carabiner_fld + "5" ).style.background = "pink";
			}
			else if(qty>=3000 && qty<=4999 ) {
				carabiner_price = numCarabinerPrice[carabiner_txt][5]["price"] * qty;
				document.getElementById( carabiner_fld + "6" ).style.background = "pink";
			}
			else if(qty>=5000 ) {
				carabiner_price = numCarabinerPrice[carabiner_txt][6]["price"] * qty;
				document.getElementById( carabiner_fld + "7" ).style.background = "pink";
			}
		}else{
			carabiner_print = 0;
			carabiner_price = 0;
		...
		if(this.vno_of_order < 1000){
			dis_c = Math.floor(before_tax * 0.15);
		}else if(this.vno_of_order >= 1000){
			dis_c = Math.floor(before_tax * 0.10);
		}
		var after_d = before_tax - dis_c;
	...
	function table_priceW(items){
		...
		if(items=="target"){
			for(rows=0;rows<7;rows++){
				document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(numUnitPrice["target"][rows]["price"] * qty[rows])+"円";
				document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(numUnitPrice["target"][rows]["price"])+"円";
				tax = Math.floor((numUnitPrice["target"][rows]["price"] * qty[rows]) * 1.1);
				document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(tax)+"円";
			}
		}else{
			switch(itemType){
				default:
				sum = Number(proto_charge + trace_charge + carabiner_price + carabiner_print);
				for(rows=0;rows<7;rows++){
					document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.ceil(((numUnitPrice["target"+QA_val][rows]["price"] * qty[rows])+Number(sum+(print_charge*qty[rows])))/qty[rows]))+"円";
					document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(Number((numUnitPrice["target"+QA_val][rows]["price"] * qty[rows])+sum+(print_charge*qty[rows])))+"円";
					tax = Math.floor((numUnitPrice["target"+QA_val][rows]["price"] * qty[rows] + sum+(print_charge*qty[rows])) * 1.1);
					document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(tax)+"円";
				} 
				break;
			}
