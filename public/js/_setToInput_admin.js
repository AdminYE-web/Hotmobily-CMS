  ...
  if ($('input[name="paper_select"]').length) {
    ...
    if (paper != undefined) {
      ...
      if (paper_color != undefined) {
        ...
        var tmp_pp = window.numUnitOptionPrice ? window.numUnitOptionPrice[dbPaperKey] : [];
  ...
  if (vno_of_order.value != "") {
    ...
    // Printed rubber
      ...
      if (silkPrintVal == "単色（シルク）印刷") {
        print_charge = 30 * parseInt(vno_of_order.value);
      } else if (silkPrintVal == "フルカラー印刷") {
        print_charge = 50 * parseInt(vno_of_order.value);
    ...
    if ($("#pcs1").length) {
      if (
        ...
        if ($("input[name=SendPrototype]:checked").val() != undefined) {
          proto_charge = Math.floor(8000);
        }
        if ($("input[name=DeFormat]:checked").val() != undefined) {
          trace_charge = Math.floor(8000);
        }
        if ($("#ariprint").length && print_umu.checked) {
          print_charge = vno_of_order.value * Math.floor(30);
        }
        if ($("#fcprint").length && print_fc.checked) {
          print_charge = vno_of_order.value * Math.floor(50);
        }
    ...
    if ($("input[name=ItemDesignVariation]:checked").length) {
      switch ($("input[name=ItemDesignVariation]:checked").val()) {
        case "2種類":
          DesignsCharge = Math.floor(3000);
          break;
        case "3種類":
          DesignsCharge = Math.floor(6000);
          break;
        case "4種類":
          DesignsCharge = Math.floor(9000);
          break;
        default:
          DesignsCharge = 0;
          break;
