function calcAcrylic() {
    ...
    var acy_size = $('input[name="acy_size"]:checked').val() || '50';

    var keyBase = 'acrylicgripholder_';
    var deliveryKey = (delivery === '6営業日') ? '6d_' : '10d_';
    var fullKey = keyBase + deliveryKey + '1side_' + acy_size;

    var priceArr = window.numUnitPrice ? (window.numUnitPrice[fullKey] || []) : [];


    // priceArr already resolved based on delivery and size
    ...
    $('#prd_size').text(acy_size + 'x' + acy_size + 'mm');
    $('#prd_delivery').text(delivery);
    $('#prd_qty').text(qty);

    var acy_part = $('input[name="acy_part"]:checked').val() || 'なし';
    $('#prd_part').text(acy_part);
}
