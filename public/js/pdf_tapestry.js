...
function validate(tmp){
  ...
  var date = y+"年"+m+"月"+d+"日";
    ...
    // Item = "オリジナルタペストリー("+$("input[name=ItemSize]:checked").val()+")";
    Item = "オリジナルタペストリー";
    itemSize = $("input[name=ItemSize]:checked").val();

    itemFabric = $("input[name=FabricType]:checked").val();
    itemDesignVa = $("input[name=ItemDesignVariation]:checked").val();
    material = itemFabric;
    ...
    // delivery = "量産品製作期間：11営業日、配送期間：1～2日"
    delivery = "量産品製作期間：5営業日（～100枚）、7営業日（～999枚）、11営業日（1000枚～）、配送期間：1～2日";
