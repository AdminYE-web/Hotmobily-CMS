...
function validate(tmp){
  ...
  var date = y+"年"+m+"月"+d+"日";
    ...
    if(document.getElementsByName("ItemPCS")[0].checked){
      ...
      $('#ItemType2').text('ラメ入り')
      itemtype2 = "ラメ入り";
      item_price = parseInt(item_price+10);
    }

    parts = $('input[name="ItemDesign"]:checked').val();
    parts_price = 500;
    ...
    if(total < 11000){
      delivery_price = 880;
      sum4_quo = formatMoney(total+delivery_price);
      sum3_quo = formatMoney(tax+(delivery_price*10/(100+10)));
    }
    ...
    $.ajax({
      ...
      data: {
        ...
        //Items data
        "Item": Item,
        "delivery": "<span style='color:red'>生産国（中国）操業規制により、通常納期+数日かかります。</span>"+delivery,
        "itemtype2": itemtype2,
