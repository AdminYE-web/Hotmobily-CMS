...
function validate(tmp){
  ...
  var date = y+"年"+m+"月"+d+"日";
    ...
    Item = "オリジナルボトルオープナー("+$("input[name=ItemPCS]:checked").val()+")";

    // ミックスカラー注文

    // オリジナルボトルオープナー(ミックスカラー注文)
    ...
    if(Item == "オリジナルボトルオープナー(ミックスカラー注文)"){
      delivery = "量産品製作期間：7営業日、配送期間：1～2日、試作品製作期間：8営業日、配送期間：1～2日";
    }else{
      delivery = "量産品製作期間：8営業日、配送期間：1～2日、試作品製作期間：8営業日、配送期間：1～2日";
    }
