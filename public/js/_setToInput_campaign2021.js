		...
		if(v=='next'){
			if(!validation_numberOf() && $('input[name=ItemPCS]:checked').val()!="ホットモバイリーファン"){
				...
				if($('input[name=ItemPCS]:checked').val()==undefined){
					...
					return false;
				}else if($('input[name=silk_print]:checked').val()==undefined){
	...
	function setToInput(){
		...
		$.get("/campaign_2021.php?type=order", function(data){
			discounts = data;
		}).done(function() {

		// Get part price
		var part = $('input[name="part"]:checked').val();
		if(part!=undefined && ItemType != "ラバーコースター"){
			var partPrice = parts_obj["part_price"];
		...
		if (ItemType == "ラバーストラップ") {
			if (vpcs1.checked||document.getElementById('pcs4').checked) {
		...
		if($('input[name=silk_print]:checked').val()!=undefined){
		...
		if((document.getElementById('pcs1').checked==true||document.getElementById('pcs4').checked) && ItemType != "スマホラバークリーナー"){
			if($('input[name=SendPrototype]:checked').val()!=undefined){ proto_charge = 8000; }
			if($('input[name=DeFormat]:checked').val()!=undefined){ trace_charge = 8000; }
			if(document.getElementById('ariprint').checked){ print_charge = vno_of_order.value * 20;}
			click_typeorder_enabled();
		}else if((document.getElementById('pcs1').checked==true||document.getElementById('pcs4').checked) && ItemType == "スマホラバークリーナー"){
			if($('input[name=SendPrototype]:checked').val()!=undefined){ proto_charge = 8000; }
			if($('input[name=DeFormat]:checked').val()!=undefined){ trace_charge = 8000; }
			click_typeorder_enabled();
		...
		// Assign Value
		var TextField7Value = vno_of_order.value * order_pcs;
		// Amount Before Tax
		var TextField9Value = TextField7Value + proto_charge + trace_charge +
		print_charge + (partPrice*vno_of_order.value) + (paperPrice*vno_of_order.value);
		if(document.getElementById('pcs4').checked){
			document.getElementById('textfield_dis').value = formatMoney(parseInt(Math.floor(TextField9Value*discounts/100)));
			TextField9Value -= parseInt(Math.floor(TextField9Value*discounts/100));
		}else{
			document.getElementById('textfield_dis').value = 0;
		}
		


		tax = Math.floor(TextField9Value * 0.1);
		total = Math.floor(TextField9Value + tax);
...
function type_special_disabled(c){
	if(c == 't1'){
		...
		$.get("/count_order.php", function(data){
			...
			// Show status
			$('.pcs_option:not(#campaign_option)').fadeIn();
			if(amount == 0){
		...
		if(document.getElementById('pcs4').checked){
			$('.pcs_option#campaign_option').fadeIn();
			$("#button_pdf2").attr('disabled',true);
		}else{
			$('#campaign_option').hide();
		...
		$('.pcs_option:not(#campaign_option)').hide();
