	// フライトタグ（刺繍タグキーホルダー）
	var omamori1 = [750,440,350,300,250,220];
	var omamori2 = [740,430,330,260,220,200];
            ...
            if((currentClick != "" && currentClick !== undefined) && (currentClick === "金額計算・見積・注文へ"))
            {
                ...
                if(Object.keys(price_value).length !== 0) 
                {
                    $.ajax({
                        ...
                        //     let header_token = $('#csrf').val() || '';
                        //     xhr.setRequestHeader('X-Header-Token', header_token);
                        // },
	...
	function validation_numberOf() {
		...
		if ( isNaN(vNumberOfOder.value) ) {
			...
			err_number.innerHTML = mess;
			return false;
		} else if( vNumberOfOder.value > 10000 ){
			mess += "<font color='red'>本数は10,000本以下で入力して下さい。10,000以上のお客様は納期をメールにてお問い合わせ下さい。</font>";
			err_number.innerHTML = mess;
			return false;
		}else {
	...
	function setToInput(){
		...
		if(vno_of_order.value != "" && $('input[name=ItemPCS]:checked').val() == "フルカラータイプ") {
			if (vno_of_order.value >= 3000) order_pcs = omamori2[5];
			else if (vno_of_order.value >= 2000) order_pcs = omamori2[4];
			else if (vno_of_order.value >= 1000) order_pcs = omamori2[3];
			else if (vno_of_order.value >= 500) order_pcs = omamori2[2];
			else if (vno_of_order.value >= 100) order_pcs = omamori2[1];
			else if (vno_of_order.value >= 50) order_pcs = omamori2[0];
		}else if(vno_of_order.value != "" && $('input[name=ItemPCS]:checked').val() == "ジャガード織タイプ") {
			if (vno_of_order.value >= 3000) order_pcs = omamori1[5];
			else if (vno_of_order.value >= 2000) order_pcs = omamori1[4];
			else if (vno_of_order.value >= 1000) order_pcs = omamori1[3];
			else if (vno_of_order.value >= 500) order_pcs = omamori1[2];
			else if (vno_of_order.value >= 100) order_pcs = omamori1[1];
			else if (vno_of_order.value >= 50) order_pcs = omamori1[0];
		}
		...
		tax = Math.floor(Math.floor((TextField7Value)*vat/(100+vat)) + Math.floor(trace_charge*vat/(100+vat)) + Math.floor(proto_charge*vat/(100+vat)) + Math.floor(mold_charge*vat/(100+vat)) + 
