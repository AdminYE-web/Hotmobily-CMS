// フライトタグ（刺繍タグキーホルダー）
var bottle = [400, 250, 120, 100, 95, 93, 89];
...
function valid_chk_btn(c) {
	if (check_val(c)) {
		...
		if ((currentClick != "" && currentClick !== undefined) && (currentClick === "金額計算・見積・注文へ")) {
			...
			let ppv = setToInputManual();
			if (Object.keys(ppv).length !== 0) {
				$.ajaxSetup({
					headers: {
						'X-CSRF-TOKEN': $('#header-token').val()
					}
...
function check_val(v) {
	...
	if ($('input[name=ItemPCS]:checked').val() == "ミックスカラー注文") {
		$('.color_area').hide();
		// ItemColors all checked false
...
function validation_numberOf() {
	...
	if (pcsValue == "色指定注文") {
		var err_cor = document.getElementById('error_pcs_col');
		err_cor.style.display = "";

		var pcs_cor = document.getElementsByClassName('ItemColors');
		var corChecked = false;

		for (var i = 0; i < pcs_cor.length; i++) {
			if (pcs_cor[i].checked) {
				corChecked = true;
				break;
			}
		}

		console.log(corChecked);

		if (!corChecked) {
			err_cor.innerHTML = "このオプションを選択してください。";
			return false;
		} else {
			err_cor.style.display = "none";
		}
...
function setToInput() {
	...
	if (vno_of_order.value != "") {
		if (vno_of_order.value >= 5000) order_pcs = bottle[6];
		else if (vno_of_order.value >= 3000) order_pcs = bottle[5];
		else if (vno_of_order.value >= 2000) order_pcs = bottle[4];
		else if (vno_of_order.value >= 1000) order_pcs = bottle[3];
		else if (vno_of_order.value >= 500) order_pcs = bottle[2];
		else if (vno_of_order.value >= 100) order_pcs = bottle[1];
		else if (vno_of_order.value >= 50) order_pcs = bottle[0];
	}
	...
	// Amount Before Tax
	TextField9Value = parseInt(TextField7Value) + proto_charge + (opp_price * vno_of_order.value);
...
function setToInputManual() {
	...
	let sku_name = "";
	if (prd == "オリジナルボトルオープナー") {
		sku_name = 'bottle-opner';
	}
