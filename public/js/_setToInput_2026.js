...
var price_coaster_standard = [
  1550, 1300, 1073, 668, 570, 570, 570, 570, 472, 391, 310, 256, 221, 185, 163,
];
...
var price_pbholder_standard = [896, 896, 500, 373, 277, 213, 178];
...
function setToInput() {
  ...
  if (vno_of_order.value != "") {
    ...
    //keykaba
      ...
      if ($("#pcs1").is(":checked") || $("#pcs4").is(":checked")) {
        if (vno_of_order.value >= 5000) order_pcs = price_keykaba_standard[6];
          ...
          order_pcs = price_keykaba_standard[5];
        else if (vno_of_order.value >= 1000)
          order_pcs = price_keykaba_standard[4];
        else if (vno_of_order.value >= 500)
          order_pcs = price_keykaba_standard[3];
        else if (vno_of_order.value >= 300)
          order_pcs = price_keykaba_standard[2];
        else if (vno_of_order.value >= 200)
          order_pcs = price_keykaba_standard[1];
        else if (vno_of_order.value >= 100)
          order_pcs = price_keykaba_standard[0];
      }
      if ($("#pcs2").is(":checked")) {
        if (vno_of_order.value >= 5000) order_pcs = price_keykaba_premium[6];
        else if (vno_of_order.value >= 3000)
          order_pcs = price_keykaba_premium[5];
        else if (vno_of_order.value >= 1000)
          order_pcs = price_keykaba_premium[4];
        else if (vno_of_order.value >= 500)
          order_pcs = price_keykaba_premium[3];
        else if (vno_of_order.value >= 300)
          order_pcs = price_keykaba_premium[2];
        else if (vno_of_order.value >= 200)
          order_pcs = price_keykaba_premium[1];
        else if (vno_of_order.value >= 100)
          order_pcs = price_keykaba_premium[0];
      }
    }
    //cleaner_rubber
    ...
    // Printed rubber
      ...
      if ($("input[name=silk_print]:checked").val() == "裏面印刷あり") {
    ...
    if ($("#pcs1").length) {
      if (
        ...
        ($("#pcs1").is(":checked") || $("#pcs4").is(":checked")) &&
        (ItemType == "ラバータグ" || ItemType == "ペットボトルホルダー")
      ) {
        if ($("input[name=SendPrototype]:checked").val() != undefined) {
          proto_charge = Math.floor(8000 * (1 + vat / 100));
        }
    ...
    if (
      ...
      $("input[name=ItemMaterial]:checked").val() == "特殊素材あり"
    ) {
    ...
    if (
      ...
      TextField7Value =
        Math.round(order_pcs * (1 + vat / 100)) * vno_of_order.value;
    } else {
      TextField7Value =
        Math.floor(order_pcs * (1 + vat / 100)) * vno_of_order.value;
    }
...
function setToInputManual() {
  ...
  if (vno_of_order.value != "") {
    ...
    if (
      $("input[name=shape_type]").length &&
      $("input[name=shape_type]:checked").val() != undefined
    ) {
    ...
    if (
      $("input[name=shape_processing]").length &&
      $("input[name=shape_processing]:checked").val() != undefined
    ) {
      $("#prd_shape_border").text($("input[name=shape_processing]:checked").val());
      $("#sample-shape").text($("input[name=shape_processing]:checked").val());
    }
...
function getPartData(v) {
  $.get("part.php", { c: "passed" }, function (data) {
    // The legacy PHP endpoint returned plain text, while Laravel returns
    // application/json and jQuery parses it before this callback runs.
    ...
    if (parts_obj) {
      ...
      console.error("Part data was not found:", v);
    }
