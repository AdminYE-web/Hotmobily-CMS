	...
	function table_priceW(items){
		...
		if(items=="strap"){
			for(rows=0;rows<7;rows++){
				if(qty[rows] < 1000){
					var total = Math.floor(numUnitPrice["strap"][rows]["price"] * 0.85);
				}else if(qty[rows] >= 1000){
					var total = Math.floor(numUnitPrice["strap"][rows]["price"] * 0.90);
				}
				document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(total * qty[rows])+"円";
				document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(total)+"円";
				tax = Math.floor((total * qty[rows]) * 1.1);
			...
			switch(itemType){
				...
				$('#qty_200').hide();
					for(rows=2;rows<7;rows++){
						if(qty[rows] < 1000){
							var total = Math.floor(((numUnitPrice[itemType+QA_val][row1]["price"] * qty2[rows])+(sum+(print_charge*qty2[rows]))) * 0.85);
						}else if(qty[rows] >= 1000){
							var total = Math.floor(((numUnitPrice[itemType+QA_val][row1]["price"] * qty2[rows])+(sum+(print_charge*qty2[rows]))) * 0.90);
						}
						document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.ceil(total/qty2[rows]))+"円";
						document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(total)+"円";
						tax = Math.floor(total * 1.1);
						document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(tax)+"円";
						row1++;
					}
				...
				for(rows=0;rows<7;rows++){
					if(qty[rows] < 1000){
						var total = Math.floor(((numUnitPrice[itemType+QA_val][rows]["price"] * qty[rows])+(sum+(print_charge*qty[rows]))) * 0.85);
					}else if(qty[rows] >= 1000){
						var total = Math.floor(((numUnitPrice[itemType+QA_val][rows]["price"] * qty[rows])+(sum+(print_charge*qty[rows]))) * 0.90);
					}
					document.getElementById( 'price_unit_' + rows ).innerHTML = formatMoney(Math.ceil(total/qty[rows]))+"円";
					document.getElementById( 'price_basic_' + rows ).innerHTML = formatMoney(total)+"円";
					tax = Math.floor(total * 1.1);
					document.getElementById( 'price_total_' + rows ).innerHTML = formatMoney(tax)+"円";
				} 
