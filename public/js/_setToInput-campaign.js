	...
	function setToInput(){
		...
		if(optStandard.checked && ItemType != "スマホラバークリーナー"){
		...
		}else if(optStandard.checked && ItemType == "スマホラバークリーナー"){
			if($('input[name=SendPrototype]:checked').val()!=undefined){ proto_charge = 8000; }
			if($('input[name=DeFormat]:checked').val()!=undefined){ trace_charge = 8000; }
			click_typeorder_enabled();
		...
		// Amount Before Tax
		var TextField9Value = parseInt(TextField7Value) + proto_charge + trace_charge +
		print_charge + (partPrice*vno_of_order.value) + (paperPrice*vno_of_order.value);

		if($('input[name=ItemPCS]:checked').val()=="ホットモバイリーファン"){
			var DisPrice = 0;
		}else{
			var DisPrice = Math.floor(parseInt(TextField9Value*0.1));
		}

		tax = Math.floor((parseInt(TextField9Value)-parseInt(DisPrice)) * 0.1);
		...
		document.getElementById('textfield9').value = formatMoney(parseInt(TextField9Value-DisPrice));
