/*
 * Complete admin calculator for create-order-rubberstrap.php.
 * This product does not depend on _setToInput_admin_2025.js or a shared
 * rubber calculator.
 */
(function (window, $) {
    'use strict';

    var PRODUCT_PREFIX = 'rubberstrap';
    ...
    function optionPrice(key, qty) {
        var price = readPrice(window.numUnitOptionPrice, key, qty);
        if (price !== null) return price;
        return readPrice(window.numUnitOptionPrice, key + '_0', qty) || 0;
    }

    function selectedPartPrice(part, qty) {
        if (!part) return 0;
        if (typeof window.getPartPriceFromGlobalArrays === 'function') {
            var globalPrice = window.getPartPriceFromGlobalArrays(part);
            if (globalPrice !== undefined && isFinite(Number(globalPrice))) return Number(globalPrice);
        }
    ...
    function setToInput() {
        ...
        var designs = parseInt(variation, 10) || 1;
        ...
        var designTotal = designs > 1
            ? (addPrice('color_variation', designs, width) || 3000 * (designs - 1))
            : 0;
        ...
        var manualTotal = manualServiceTotal(qty);
        var total = baseTotal + partTotal + paperTotal + silkTotal + coatingTotal + materialTotal
            + designTotal + prototypeTotal + traceTotal + manualTotal;

        setText('#prd_ItemPCS', itemPcs);
        setText('#prd_ItemSize', itemSize);
        ...
        setValue('#display_ItemPCSFee', money(0));
        setValue('#display_ItemSizeFee', money(0));
        setValue('#textfield13', money(silkTotal));
        setValue('#textfield7_1', money(partTotal));
