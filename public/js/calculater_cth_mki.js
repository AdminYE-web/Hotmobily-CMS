...
var i4_std_2 = [];
  ...
  function setup_price(type){

    if(type == "バージン材"){
      // MKI Price
      i1_std_1 = [292,346,208,165,136,131];
      i1_std_2 = [318,272,231,184,155,147];
    
      i2_std_1 = [321,277,239,194,162,156];
      i2_std_2 = [347,299,262,210,180,168];
    
      i3_std_1 = [0,188,160,127,107,99];
      i3_std_2 = [0,208,178,142,119,110];
    
      i4_std_1 = [0,216,185,147,121,112];
      i4_std_2 = [0,233,201,158,135,125];
    }else{
      // MKI-50 Price
      i1_std_1 = [306,255,218,173,144,136];
      i1_std_2 = [332,285,242,194,162,154];
    
      i2_std_1 = [330,286,246,199,166,162];
      i2_std_2 = [358,310,274,220,186,176];
    
      i3_std_1 = [0,199,169,134,112,105];
      i3_std_2 = [0,219,188,150,125,117];
    
      i4_std_1 = [0,224,193,153,129,119];
      i4_std_2 = [0,246,210,166,143,133];
    }
  }
    ...
    if(ItemType == "リサイクル材"){
      ...
      $('.mki-only').show();
			if($("input[name=cth_type]:checked").val()=="昇華転写片面印刷" || $("input[name=cth_type]:checked").val()=="昇華転写両面印刷"){
				$.each($('input[name=cth_color]'), function(){
					if($(this).val()!="05WHITE"){
						$(this).closest('.part-content').hide();
					}else{
				...
				$.each($('input[name=cth_color]'), function(){
					if(!$(this).closest('.part-content').is(':visible')){
						...
						$("input[name=cth_color]:first").prop('checked', true);
					}
				});
		...
		switch($('input[name=cth_type]:checked').val()){
			...
			case "エンボス加工":
				$('#type-part-pic').attr('src','img/cth4.jpg');
	...
	function validation_numberOf() {
		...
		if ( isNaN(vNumberOfOder.value) ) {
			...
			err_number.innerHTML = mess;
			return false;
		}else if(($('input[name=cth_type]:checked').val()=="1色印刷（シルクスクリーン）" || $('input[name=cth_type]:checked').val()=="エンボス加工") && vNumberOfOder.value < 300){
    ...
    setup_price(ItemType);
		...
		switch($('input[name=cth_type]:checked').val()){
			...
			case "1色印刷（シルクスクリーン）": var prd_basic_price = 4950;break;
			case "エンボス加工": var prd_basic_price = 7000;break;
		}

		$('#sample-prd-prdt').text($('input[name=item_type]:checked').val()+"("+$('input[name=cth_type]:checked').val()+")");
		$('#prd_production').text($('input[name=item_type]:checked').val()+"("+$('input[name=cth_type]:checked').val()+")");

    $('#sample-prd-color').text($('input[name=cth_color]:checked').val());
		$('#prd_color').text($('input[name=cth_color]:checked').val());

    $('#sample-prd-sewing-color').text($('select[name=cth_sewing_color]').val());
		$('#prd_sewing_color').text($('select[name=cth_sewing_color]').val());
...
function setzero(){
	...
	$("input[name=ItemType]:first").prop('checked', true);
	$("input[name=cth_type]:first").prop('checked', true);
	$("input[name=cth_color]:first").prop('checked', true);
	$("select[name=cth_sewing_color]:first").prop('selected', true);
