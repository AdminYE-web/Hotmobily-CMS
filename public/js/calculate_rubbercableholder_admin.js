/**
 * calculate_rubbercleaner_admin.js
 * Admin-side pricing calculator for スマホラバークリーナー.
 *
 * - Product unit prices are fetched from window.numUnitPrice (DB-sourced via create-order-rubbercleaner.php)
...
/*
 * Product-local price engine for create-order-rubbercableholder.php.
 * Do not move this lookup to a shared rubber calculator: each order form
 * owns its product prefix and fallback policy.
 */
function rbCableHolderReadPrice(pool, key, qty) {
...
function rbCableHolderAddPrice(code, qty) {
    ...
    var service = normalized.replace(/^rubbercableholder_/, '');
    var candidates = [];
    function add(key) { if (key && candidates.indexOf(key) === -1) candidates.push(key); }
    add(requested);
    add(normalized);
    add('rubbercableholder_' + service);
    add(service);
    ...
    for (var i = 0; i < candidates.length; i++) {
        var price = rbCableHolderReadPrice(pool, candidates[i], qty || 1);
        if (price !== null) return price;
    }
    return 0;
}

function rbCableHolderManualTotal() {
 ...
 */
function get_cableholder_price() {
    var itemPCS = $('input[name="ItemPCS"]:checked').val() || 'スタンダード';
    var dbKey = 'rubbercableholder_standard';
    if (itemPCS === 'プレミアム') {
        dbKey = 'rubbercableholder_premium';
    } else if (itemPCS === 'スタンダード（スピード7営業日発送）') {
        // Speed has a dedicated DB product.  Fall back to standard only when
        // an older database does not contain that product yet.
        dbKey = 'rubbercableholder_standard_speed';
    }
...
function get_add_price(code) {
    return rbCableHolderAddPrice(code, 1);
}
...
function setToInput() {
    ...
    var silkPrintVal = $('input[name="silk_print"]:checked').val() || '印刷なし';
    ...
    var sendPrototype = $('input[name="SendPrototype"]:checked').val() || $('input[name="SendPrototype"][type="checkbox"]:checked').val() || 'なし';
    var deFormat = $('input[name="DeFormat"]:checked').val() || $('input[name="DeFormat"][type="checkbox"]:checked').val() || 'なし';
    ...
    var price_tiers = get_cableholder_price();
    unit_price = (qty > 0) ? lookupPrice(price_tiers, qty) : 0;
    var hasDedicatedSpeedPrice = itemPCS === 'スタンダード（スピード7営業日発送）'
        && window.numUnitPrice
        && Array.isArray(window.numUnitPrice.rubbercableholder_standard_speed)
        && window.numUnitPrice.rubbercableholder_standard_speed.length > 0;
    if (itemPCS === 'スタンダード（スピード7営業日発送）' && !hasDedicatedSpeedPrice) {
        // According to _setToInput_2025.js, speed delivery adds 30, 44, 57 depending on qty
        if (qty >= 300) unit_price += 30;
        else if (qty >= 200) unit_price += 44;
        else if (qty >= 100) unit_price += 57;
    }
    var strapPrice = unit_price * qty;

    // ---- 1.5 Silk Print price ----
    var silkPrintPrice = 0;
    if (itemPCS !== 'プレミアム') {
        if (silkPrintVal === '単色（シルク）印刷') {
            var singlePrice = get_add_price('rubbercableholder_backprinting_single') || get_add_price('back_printing_single');
            var platePrice = get_add_price('silk_plate') || 0;
            silkPrintPrice = singlePrice + platePrice;
        } else if (silkPrintVal === 'フルカラー印刷') {
            silkPrintPrice = get_add_price('rubbercableholder_backprinting_full') || get_add_price('back_printing_full');
        }
    }
    var silkPrintTotal = silkPrintPrice * qty;

    // ---- 2. Attachment (part) price ----
    partPrice = 0;
    if (part && part !== '') {
        var partKey = part;
    ...
    var partTotal = partPrice * qty;

    // ---- 3. Coating price ----
    coatingPrice = 0;
    if (coating === '汚れ防止加工あり') {
        coatingPrice = get_add_price('rubbercableholder_antistain') || get_add_price('anti_stain') || get_add_price('coating') || get_add_price('rubber_coating');
    }
    ...
    // ---- 5. Prototype (試作品) ----
    proShipping = 0;
    var isSendPrototype = (sendPrototype === 'あり');
    if (isSendPrototype) {
        proShipping = get_add_price('rubbercableholder_prototype') || get_add_price('rubber_prototype');
        if (proShipping <= 0) proShipping = get_add_price('prototype');
    ...
    traceCharge = 0;
    var isDeFormat = (deFormat === 'あり');
    if (isDeFormat) {
        traceCharge = get_add_price('rubbercableholder_datatrace') || get_add_price('rubber_data_trace');
        if (traceCharge <= 0) traceCharge = get_add_price('data_trace');
    ...
    if (designNum > 1) {
        var variationUnitPrice = get_add_price('rubbercableholder_colorvariation') || get_add_price('rubber_color_variation') || get_add_price('color_variation');
    ...
    var materialKey = '';
    switch (itemMaterial) {
        case '特殊素材：蓄光（オレンジ色）': materialKey = 'rubbercableholder_phosphorescent_orange'; break;
        case '特殊素材：蓄光（緑色）': materialKey = 'rubbercableholder_phosphorescent_green'; break;
        case '特殊素材：蛍光': materialKey = 'rubbercableholder_fluorescent'; break;
        case '特殊素材：ラメ': materialKey = 'rubbercableholder_glitter'; break;
        case '特殊素材：金色': materialKey = 'rubbercableholder_gold'; break;
        case '特殊素材：銀色': materialKey = 'rubbercableholder_silver'; break;
    }
    if (materialKey !== '') {
    ...
    // ---- Subtotal ----
    var manualServiceTotal = rbCableHolderManualTotal();
    sub_total = strapPrice + partTotal + silkPrintTotal + coatingTotal + paperTotal + proShipping + traceCharge + designsCharge + materialCharge + manualServiceTotal;
    disPrice = 0;
    total = sub_total - disPrice;

    // ---- Summary display (spec table) ----
    $('#prd_ItemPCS').text(itemPCS);
    $('#prd_silk_print').text(silkPrintVal);
    ...
    var $t13 = document.getElementById('textfield13'); if ($t13) $t13.value = silkPrintTotal;
    var $t7_1 = document.getElementById('textfield7_1'); if ($t7_1) $t7_1.value = partTotal;
    ...
    $('input[name="SilkPrint"]').val(silkPrintTotal);
    $('input[name="PartPrice"]').val(partTotal);
...
// Alias so customer-form onclick="check_val(...)" still triggers recalc
window.setToInput = setToInput;

// Attachment part data handler (called by getPartData in customer form)
function getPartData(partName) {
    if (!partName) return;
    var partRadio = $('input[name="part"][value="' + partName + '"]');
    var price = parseFloat(partRadio.data('price') || 0);
    partPrice = isNaN(price) ? 0 : price;
    setToInput();
}
...
$(document).ready(function () {
    ...
    var checkedPart = $('input[name="part"]:checked');
    if (checkedPart.length) {
        partPrice = parseFloat(checkedPart.data('price') || 0);
    }
