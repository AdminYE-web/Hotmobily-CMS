...
/*********** input validation ***********/ 
function validation(){	
        var color_result = validation_color();
        var number_result = validation_numberOf();
        var prototype_result = validation_prototype();


	if (color_result && number_result && prototype_result ) {
		return false;}
	else{
        return true;
	}
}
function validation_color() {
	//入力値
	var vcolor = document.getElementById('color');
	
	//エラー出力用id
	var err_color = document.getElementById('err_color_mess');
	var mess = "";

	err_color.style.display = "";

	if ( isNaN(vcolor.value) ) {
		mess += "<font color='red'>半角数値以外が入力されています。</font>";
		err_color.innerHTML = mess;
		return false;
	} else if(vcolor.value == '' || vcolor.value < 1 || vcolor.value > 8){
		mess += "<font color='red'>1色～8色まででご指定下さい。</font>";
		err_color.innerHTML = mess;
		return false;
	} else {
...
function validation_numberOf() {
	...
	if (cleaner_rubber.checked) {
		if (vNumberOfOder.value == '' ||  vNumberOfOder.value < 200) {
			mess += "<font color='red'>本数は２００本以上で入力して下さい</font>";
	...
	if ( isNaN(vNumberOfOder.value) ) {
		...
		mess += "<font color='red'>本数は５０本以上で入力して下さい。</font>";
		err_number.innerHTML = mess;
		return false;
