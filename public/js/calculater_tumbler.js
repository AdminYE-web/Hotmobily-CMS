...
var priceA = [2354, 2207, 1895, 1518, 1215, 1168, 1141, 1135, 1117, 1109];
var priceB = [2722, 2653, 2613, 2327, 1637, 1611, 1588, 1539, 1494, 1473];
...
function isPromoPeriod() {
  ...
  // แปลงเวลาปัจจุบันให้เป็นเวลาญี่ปุ่น
  var japanNow = new Date(
    now.toLocaleString("en-US", { timeZone: "Asia/Tokyo" }),
  );

  var start = new Date(japanNow.getFullYear(), 3, 20, 0, 0, 0); // 20 เม.ย. 00:00:00
  var end = new Date(japanNow.getFullYear(), 4, 31, 23, 59, 59); // 31 พ.ค. 23:59:59
...
function validation_numberOf() {
  ...
  if (
    $("input[name=shape_processing]:checked").val() == "フルカラー（3DUV印刷）"
  ) {
  ...
  if (isNaN(vNumberOfOder.value)) {
    ...
    return false;
  } else if (vNumberOfOder.value == "" || vNumberOfOder.value < 10) {
    mess += "<font color='red'>本数は10本以上で入力して下さい。</font>";
    err_number.innerHTML = mess;
    return false;
  } else if (vNumberOfOder.value > 2000) {
    mess +=
      "<font color='red'>本数は2,000本以下で入力して下さい。2,000以上のお客様は納期をメールにてお問い合わせ下さい。</font>";
    err_number.innerHTML = mess;
    return false;
  } else {
...
function setToInput() {
  ...
  if (
    vno_of_order.value != "" &&
    $("input[name=shape_processing]:checked").val() == "レーザー加工"
  ) {
    if (vno_of_order.value >= 2000) {
      order_pcs = priceA[9];
  ...
  if (
    ...
    $("#sample-processing").text(
      $("input[name=shape_processing]:checked").val(),
    );
  ...
  if ($("input[name=SendPrototype]:checked").val() != undefined) {
    // Promotional period 20/04 – 31/05: prototype is free
    proto_charge = isPromoPeriod() ? 0 : 4950;
  ...
  if ($("input[name=DeFormat]:checked").val() != undefined) {
    ...
    // Promotional period 20/04 – 31/05: data trace is free
    trace_charge = isPromoPeriod() ? 0 : 2200;
  } else {
  ...
  total =
    parseInt(TextField9Value) - parseInt(DisPrice) + parseInt(coating_price);

  document.getElementById("coating_shape_price").value =
    coating_price.toLocaleString("en");
